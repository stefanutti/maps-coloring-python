...
/* plantri.c :  generate imbedded planar graphs 
  ...
  Authors:  Gunnar Brinkmann   Gunnar.Brinkmann@ugent.be
            Brendan McKay      Brendan.McKay@anu.edu.au
            Heidi Van den Camp

  plantri is subject to the copyright and license detailed in
  plantri-guide.txt.
...
---------
   ...
   Output formats: 
      ...
      ascii format: 
         ...
         This is the same as planar_code but human-readable.

      edge_code:
         See the manual.  At the moment there is a limit of 255 on the
      ...
      For graphs with multiple loops, edge_code and text edgecode are
      the only output formats which are completely unambiguous.
...
The following table gives the largest legal MAXN value.
 ...
 Switches -E and -Ed give edge_code output.  These have no limit on the
 number of vertices but (currently) a limit of 255 on the number of
 undirected edges.

 Switches -T and -Td give double_code output. See above and the guide.
...
Change History:
       ...
       28-Feb-2018 : Move secret -2 to public -X, which increases the
                       splitting level by 1.
                     Increase default splitting levels for most options.
                     Fix -m4c3x, which didn't work as advertised.
                     Add -T for double code output.
                     Fixed the group of the gyro (triangulation with
                        three vertices and one loop)
                     -- making 5.1

        1-Jul-2021 : forbid -c4x and -c5x, which never meant anything
                   : Use long long for counters
                     -- making 5.2

       23-Aug-2021 : Fix an erroneous splitting level for quadrangulations
                     and bipartite polytopes (-q and -bp options). It
                     caused a few outputs to be written more than once
                     when the res/mod option was used. The bug did not
                     occur in version 5.0 or earlier.  Version 5.1 was
                     not released.
                   : Remove the erroneous reference to CLOCKS_PER_SEC.
                     -- making 5.3

        1-Mar-2023 : Added -Q code
                     -- making 5.4

       17-May-2024 : Fixed -p splitting for 2 or 3 vertices.
                     -- making 5.5
...
#define quadr_P2(n) (edges - 56 + 4*MAXN + 8*(n))
/* edges +8 + 4*MAXN + 8*(n) - 8*8, again since the smallest
   possible n is 8 */
#define quadr_P3(n) (edges - 184 + 12*MAXN + 16*(n))
/* edges - 56 + 12*MAXN + 16*(n) - 8*16 */
...
/* In versions up to 5.0, large integers were represented as a
 * structure of two longs, due to "long long" once being of
 * uncertain availability.
 *
 * Starting at version 5.1, we assume "long long".
 *
 * All the macros are left here in case some code uses them,
 * but plantri itself now uses only PRINTBIG.
 *
*/

typedef unsigned long long bigint;
#define ZEROBIG(big) big = 0
#define ISZEROBIG(big) ((big) == 0)
#define SETBIG(big,value) big = (value)
#define ADDBIG(big,extra) big += (extra)
#define PRINTBIG(file,big) fprintf(file,"%llu",(big))
#define BIGTODOUBLE(big) ((double)(big))
#define SUMBIGS(big1,big2) big1 += (big2)
#define SUBBIGS(big1,big2) big1 -= (big2)
#define ISEQBIG(big1,big2) ((big1) == (big2))
...
#define COUNTSWITCH(name,var)  \
    else if (arg[j]==name) {CHECKSWITCH(name); ++var;}
#define INTSWITCH(name,var)  \
    else if (arg[j]==name) {CHECKSWITCH(name); var = getswitchvalue(arg,&j);}

#define SECRET_SWITCHES "01"
   ...
   to reduce5() */
{
    ...
    list[0] = e->next; list[1] = e->next->invers;
    list[2] = e->next->next; list[3] = list[2]->invers;
...
/* rotating e */
    work1=e->next->invers; work2=work1->next;
    e->start=work1->start; e->end=e->prev->end;
    e->prev=work1; e->next=work2;
    work1->next=e; work2->prev=e;
    (degree[work1->start])++;
...
/* rotating e */
    work2=e->prev->invers; work1=work2->prev;
    e->start=work1->start; e->end=e->next->end;
    e->prev=work1; e->next=work2;
    work1->next=e; work2->prev=e;
    (degree[work1->start])++;
    ...
    for (b = 1; b < nv-1; ++b)
    {
        ...
        do
        {
            if (e->end > b && degree[e->end] >= 4)
            {
                ...
                eclast = eclast->prev;
                for (; ec != eclast; ec = ec->next)
                    if (ISMARKED_V(ec->end)) return TRUE;
            }
            e = e->next;
        } while (e != elast);
    }
...
numbermarked(void)
{
  ...
  while (index<number)
    ...
    for (i=0;i<degree[dummy];i++, run=run->next)
      ...
      { MARK_V(buffer); list[number]=buffer; number++; }
      }
    index++;
    }
  return number;
...
/* Make an octahedron using the first 24 edges */
{
   ...
   EDGE *buffer;
    ...
    for (i=0; i<=5; i++)
      { buffer=edges+4*i;
        firstedge[i]=buffer; degree[i]=4;
        buffer->next=buffer+1; buffer->prev=buffer+3; buffer->start=i;
        buffer++; buffer->next=buffer+1; buffer->prev=buffer-1; buffer->start=i;
        buffer++; buffer->next=buffer+1; buffer->prev=buffer-1; buffer->start=i;
        buffer++; buffer->next=buffer-3; buffer->prev=buffer-1; buffer->start=i;
      }
...
*/
    ...
    for (n=6; n<=MAXN-2; n++)
      { run=start=P_op(n);
        run->start=n; run->end=n+1; run->min=run;
            run->invers=start+1; run->next=start+2; run->prev=start+4;
        run=start+1; run->start=n+1; run->end=n;
            run->min=run->invers=start; run->next=start+7; run->prev=start+11;
        run=start+2; run->start=n; run->min=run; run->invers=start+3;
            run->prev=start;
        run=start+3; run->end=n; run->min=run->invers=start+2;
            run->next=start+10;
        run=start+4; run->start=n; run->min=run; run->invers=start+5;
            run->next=start; 
        run=start+5; run->end=n; run->min=run->invers=start+4;
            run->prev=start+6;
        run=start+6; run->end=n+1; run->min=run; run->invers=start+7;
            run->next=start+5; 
        run=start+7; run->start=n+1; run->min=run->invers=start+6;
            run->next=start+9; run->prev=start+1;
        run=start+8; run->end=n+1; run->min=run; run->invers=start+9; 
        run=start+9; run->start=n+1; run->min=run->invers=start+8;
            run->next=start+11; run->prev=start+7;
        run=start+10; run->end=n+1; run->min=run; run->invers=start+11;
            run->prev=start+3;
        run=start+11; run->start=n+1; run->min=run->invers=start+10;
            run->next=start+1; run->prev=start+9;
      }
...
*/
   ...
   for (n=6; n<=MAXN-1; n++)
     { run=start=Q_op(n);  run->min=run;
                   run->start=n; run->invers=start+1; run->next=start+4;
       run=start+1; run->end=n; run->invers=run->min=start;
           run->prev=start+2;
       run=start+2; run->invers=start+3; run->next=start+1; run->min=run;
       run=start+3; run->invers=run->min=start+2; run->prev=start+5;
       run=start+4; run->start=n; run->invers=start+5; run->prev=start;
           run->min=run;
       run=start+5; run->end=n; run->invers=run->min=start+4;
           run->next=start+3;
     }
    ...
    for (n=6; n<=MAXN; n++)
      { start=four_op(n);
        for (i=0, run=start; i<6; i+=2)
          { run->start=n; run->min=run; run->invers=run+1; run++;
            run->end=n; run->invers=run->min=run-1; run++; }
        run=start; run->next=run+2; 
        run+=2; /*2*/ run->prev=start; run->next=run+2;
        run+=2; /*4*/ run->prev=start+2; 
      }
...
/* Then initialise the edges for the S-operation. */
    ...
    for (n=6; n<=MAXN-3; n++)
      { start=S_op(n);
        for (i=0;i<18;i+=2)
          { run=start+i; run->invers=run+1; run->min=run; 
            run++; run->invers=run->min=run-1; }

        run=start;    run->start=n+1; run->end=n; run->next=start+4;
                      run->prev=start+9;
        run++; /*1*/  run->start=n; run->end=n+1; run->next=start+11;
                      run->prev=start+3;
        run++; /*2*/  run->start=n+2; run->end=n; run->next=start+15;
                      run->prev=start+5;
        run++; /*3*/  run->start=n; run->end=n+2; run->next=start+1;
                      run->prev=start+13;
        run++; /*4*/  run->start=n+1; run->end=n+2; run->next=start+7;
                      run->prev=start;
        run++; /*5*/  run->start=n+2; run->end=n+1; run->next=start+2;
                      run->prev=start+17;
        run++; /*6*/  run->end=n+1; run->next=start+16; 
        run++; /*7*/  run->start=n+1; run->next=start+9; run->prev=start+4;
        run++; /*8*/  run->end=n+1; run->prev=start+10;
        run++; /*9*/  run->start=n+1; run->next=start; run->prev=start+7;
        run++; /*10*/ run->end=n; run->next=start+8; 
        run++; /*11*/ run->start=n;  run->next=start+13; run->prev=start+1;
        run++; /*12*/ run->end=n; run->prev=start+14;
        run++; /*13*/ run->start=n; run->next=start+3; run->prev=start+11;
        run++; /*14*/ run->end=n+2; run->next=start+12;
        run++; /*15*/ run->start=n+2; run->next=start+17; run->prev=start+2;
        run++; /*16*/ run->end=n+2; run->prev=start+6;
        run++; /*17*/ run->start=n+2; run->next=start+5; run->prev=start+15;
      }
...
*/
{
    ...
    if (missing_vertex>=0) limitnv=nv+1; else limitnv=nv;
    ...
    if (code_edge==NULL) run=firstedge[0]; 
    else { run=code_edge; }
    ...
    for (j=1; j<degree[startvertex]; j++, run=run->next)
      {
        if (ISMARKED(run)) { *code = run->index; code++; }
        else { 
               MARK(run->invers);
               run->invers->index= *code =edgenumber; code++;
               edgenumber++;
             }
      }
    *code=255; code++;
...

{
    ...
    if (code_edge==NULL) start=firstedge[0]; 
    else { start=code_edge; }
    run=start;
    edgenumber=0;
    do
      {
        if (ISMARKED(run)) { *code=run->index; code++; }
          ...
          { 
            MARKLO(run->invers); 
          /* numbered but left hand face not yet listed. In case of bridges it
             can even happen for the first face that the other side is already
             marked high.*/
    ...
    *code=255; code++;
    ...
    /* that was to make sure that in the case of disc triangulations the
       right start is taken -- now the rest */
      ...
      while(newcode<last)
        {
          ...
          for (run--; run!=start; run--, newcode++) *newcode=*run;
          *newcode=255; newcode++;
          start=end;
        }
      return;
  ...
  while(newcode<last)
    {
      ...
      for ( run--; run!=start; run--, newcode++) *newcode=number[*run];
      *newcode=255; newcode++;
      start=end;
    }
  return;
...
static void
write_double_code(FILE *f, int doflip)

/* Write both the primal and code graphs in edge_code using A,B,C,... for
   edges.  This uses the switch dswitch to determine which of the two codes
   is written first. */
{
    unsigned char code[MAXN+MAXE+MAXF+6];
    int i,nvlim,edgename,len,nf,pass,first;
    EDGE *e,*elast,*ee,*eelast;

    if (ne/2 >= 189)
    {
        fprintf(stderr,
         ">E %s: Output routine write_double_code not prepared for ",cmdname);
          fprintf(stderr,"that many edges\n");
          exit(1);
    }
    
    nvlim = (missing_vertex < 0 ? nv : nv+1);
    nf = 2 + (ne/2) - nv;
    ...
    {
        if ((!dswitch && pass == 0) || (dswitch && pass == 1))
        {
              /* primal */

            if (nv >= 100) code[len++] = '0' + (nv/100);
            if (nv >= 10) code[len++] = '0' + ((nv/10)%10);
            code[len++] = '0' + (nv%10);
            code[len++] = ' ';
        ...
        else
        {
            ...
            if (nf >= 100) code[len++] = '0' + (nf/100);
            if (nf >= 10) code[len++] = '0' + ((nf/10)%10);
            code[len++] = '0' + (nf%10);
            code[len++] = ' ';
    ...
    if (fwrite(code,sizeof(unsigned char),len,f) != len)
    {
        fprintf(stderr,">E %s: fwrite() failed\n",cmdname);
        perror(">E ");
        exit(1);
...
static void
write_double_code_int(FILE *f, int doflip)

/* Outputs a nested list of faces.
   Without -d: each face is a vertex rotation of the primal (= face of dual),
     using primal vertex IDs as edge endpoints.
   With -d: each face is a face of the dual graph (= primal vertex rotation),
     using dual vertex IDs (= primal face IDs from make_dual()) as edge
     endpoints. The resulting graph is 3-regular. */
{
    int i,nvlim,first_vertex;
    EDGE *e,*elast;

    nvlim = (missing_vertex < 0 ? nv : nv+1);

    if (dswitch) make_dual();
    ...
    for (i = 0; i < nvlim; ++i)
    {
        ...
        do {
            if (dswitch)
                fprintf(f, "[%d,%d]", e->rightface, e->next->rightface);
            else
                fprintf(f, "[%d,%d]", e->end, e->next->end);
            e = e->next;
            if (e != elast) fprintf(f, ",");
        } while (e != elast);
...

{
    ...
    *code=nf; code++;
    ...
    run=givenedge;
    do { MARKLO(run); run->rf=1; run=run->invers->prev;
       } while (run!=givenedge);
    if (!ISMARKED(givenedge->invers)) /* no loop in the dual at this point */
      { run2=run=givenedge->invers;
        do { MARKLO(run2); run2->rf=2; run2=run2->invers->prev;
           } while (run2!=run);
        last_number = 2;
        ...
        for (run = temp->invers->next; run != temp; run = run->invers->next)
          ...
          { if (!ISMARKED(run))
            { startedge[last_number] = run->invers;
              ...
              run2=run;
              do
                { MARKLO(run2); run2->rf=last_number;
                  run2=run2->invers->prev;
                } while (run2!=run);
              *code = last_number;
            }
...
#define CODE0(x) ((x)<10 ? '0'+(x) : 'a'+(x)-10)
    ...
    if (missing_vertex < 0) lastnum = nv-1;
    else                    lastnum = nv;
...
static void
check_it(int code, int facesize)

/* Checks these properties:
   1. Degrees are correct.
   2. Faces are all of size facesize (3 or 4, ignore if 0)
...

{
    ...
    for (i = 0; i < nv; ++i)
    {
        ...
        for (j = 0; j < degree[i]; ++j)
        {
            ...
            {
                fprintf(stderr,">E non-triangular face, code=%d\n",code);
                exit(1);
            }
            if (facesize == 4)
                if (e->invers->next == e
                    || e->invers->next->invers->next == e
                    || e->invers->next->invers->next->invers->next == e
                    || e->invers->next->invers->next->invers->next->invers->next != e)
            {
                fprintf(stderr,">E non-quadrangular, code=%d\n",code);
                exit(1);
            }
   ...
   purpose is to write the graph and to collect some stats. */
{
    ...
    if (xswitch && connec != xconnec) return;
    ...
    ++nout[connec];
    if (oswitch) nout_op[connec] += wt;

    if (pswitch)
    {
        if (oswitch) nout_e_op[ne/2] += wt;
        ++nout_e[ne/2];
    }

    if (polygonsize == 0)
    {
        if (oswitch) nout_p_op[outside_face_size] += wt;
        ++nout_p[outside_face_size];
    }
    ...
    if (polygonsize < 0)
    {
        ...
        numrooted += numroot;
        if (pswitch) numrooted_e[ne/2] += numroot;
        mind = degree[0];
        for (ii = 1; ii < nv; ++ii) if (degree[ii] < mind) mind = degree[ii];
        if (degree[nv-1] < 6) nummindeg[mind] += wt;
    }
    else
    {
        numroot = wt * numorbitsonface(nbtot,nbop,code_edge);
        numrooted += numroot;
        numbigface[degree[missing_vertex]] += numroot;
    }
    if (needgroup && nbtot == 1) ntriv += wt;
#ifdef STATS2
    num2 = 0;
    for (i = 0; i < nv; ++i) if (degree[i] == 2) ++num2;
    numtwos[num2] += wt;
#endif
#endif
   ...
   (This represents the group, but not a canonical labelling.) */
{
    ...
    *xnbtot = 1;
    ...
    for (i = 0; i < ne; ++i) nb[i]->index = i;

    for (j = 1; j < nbtot; ++j)
    {
        nb = (EDGE**)old_numbering[j];
        for (i = 0; i < ned; ++i) image[i] = nb[ed[i]->index]->min;
    ...
    while (start != end)
      { run=start->invers;
        ...
        for ( ; run != start; run=run->prev->invers)
            if (ISMARKED_V(run->start)) return 0; 
      }
...

{
    ...
    if (end == e->prev) return 0;
    end = end->invers;

    v = e->prev->end;
    if (degree[v] == 2) return 1;

    for (run = e->invers->prev; run != end; run = run->invers->prev)
        if (run->end == v) return 0;
...

{
    ...
    for (i=0; i<numold; i++, old++)
      { e = *old; 
        if (degree[e->start] > minpolydeg && degree[e->end] > minpolydeg
             && (e->left_facesize == 3 || e->invers->left_facesize == 3))
            newe[counter++]=e;
      }
    ...
    for (i=0; i<num_loops; i++)
      { run=loops[i];
        ...
        if (ISMARKED_V(run->start)) /* already another loop started
          ...
          { 
            if (ISMARKED(run->prev->prev) && (degree[run->prev->end]>minimumdeg)
                && (run->prev->start != run->prev->end))
            ...
            if (ISMARKED(run->next->next) && (degree[run->next->end]>minimumdeg)
                && (run->next->start != run->next->end))
...

{
    ...
    if ((num_doubleedges<2) || 
        ((minimumdeg>=3) && (num_doubleedges<4))) { *number_pos_loops=0; return; }
...
/* Now all the edges contained in doubleedges are marked and we can
   ...
   double edges. */
    ...
    {   /* there must be a neighbouring doubleedge */
        run= doubleedges[i];
        if (ISMARKEDLO(run->next) && (!ISMARKEDHI(run->invers->prev))
                    && (run->start == run->invers->prev->prev->end))
        ...
        if (ISMARKEDLO(run->next) && (!ISMARKEDHI(run->invers->prev))
                    && (run->start == run->invers->prev->prev->end))
        {   buffer=run->invers->prev;
            MARKHI(buffer);  MARKHI(buffer->invers);
        possible_loops[counter]=buffer->min;
        counter++;
        }
    }
...
*/
    ...
    if (minimumdeg==1)
      for (i=0;i<nv;i++)
        ...
        if (degree[i]==2)
          { 
            possible_loops[counter]=(firstedge[i])->min; counter++;
            possible_loops[counter]=(firstedge[i])->next->min; counter++;
          }
        }
...

{
    ...
    for (i = 0; i < oldnum; ++i)
    {
        ...
        {
            edge[newnum++] = edge[i];
            for ( ; nb < nblim; nb += MAXE) MARKLO((*nb)->min);
        }
    }
    *nedges = newnum;
}
   ...
   The loops are all in min form. */
{
    ...
    if (!(PRE_FILTER_SPECIALLOOP)) return;
#endif

    find_special_loopmakers(loop,nloops,loopmaker,&nloopmakers);
    if (nloopmakers == 0) return;
 
    if (nbtot > 1) prune_edgelist(loopmaker,&nloopmakers,nbtot);
 ...
 */
    ...
    if ((numdoubles<2) || 
        ((minimumdeg>=3) && (numdoubles<4))) return;
    ...
    {
        for (i = 0; i < nloopmakers; ++i) loopmaker[i]->index = i;
        for (i = 0; i < nloopmakers; ++i)
        {
        ...
        for (x = 1; x < xlim; ++x)
        {
            ...
            // isloop[j] = !isloop[j];
            if (isloop[j] == 0)
                if (degree[loopmaker[j]->start] == 1
                              || degree[loopmaker[j]->end] == 1)
                {
                    isloop[j] = 1;
                    ++numpseudo;
                }
            ...
            for (j = 0; j < nv; ++j) if (degree[j] < minimumdeg) break;
            if (j < nv) continue;
            ...
            else
            {
                for (j = 0; j < nloops; ++j)
                {
                   if (loop[j]->next->next->next->invers == loop[j]
                         || loop[j]->prev->prev->prev->invers == loop[j])
                       break;
                }
    ...
    else   /* Case of non-trivial group */
    {
        ...
        if (isloop[nloopmakers-1] == 2) switch_edge_back(loopmaker[nloopmakers-1]);
    }
}
        ...
        for (j=degree[i]; j; j--, run=run->next)
            if (run==run->min && degree[run->end]>=degneeded)
            {
                ...
                {   dummy = firstedge[run->next->end];
                    while (dummy->end != run->prev->end) dummy = dummy->next;
                    list[num] = run; 
...
/* Add more comments.  All edges in the parameters are in min form. */
{
    ...
    a=lastflipped->start; MARK_V(a);
    b=lastflipped->end; MARK_V(b);
    c=lastflipped->prev->end; MARK_V(c);
    d=lastflipped->next->end; MARK_V(d);
...

{
    ...
    if (nbtot == 1) 
        for (i = 0; i < count; ++i) feasible[i] = 1;
    else
    {
        nb0 = (EDGE**)numbering[0];
        nblim = (EDGE**)numbering[nbtot];
    ...
    for (i = 0; i < count; ++i)
        if (feasible[i])
        {
            ...
            { 
                ...
                if (matecol < flipcol) feasible[i] = 0;
            }
        }
}
   ...
   Note that the definition of colour is used also in find_feasible_flips. */
{
    ...
    col1 = col2 = (long)(degree[test_edge->prev->end]
                                + degree[test_edge->next->end]) << 20;
    ...
    num_good = 0;
    if (col1 == bestcol) good[num_good++] = test_edge;
    if (col2 == bestcol) good[num_good++] = test_edge->invers;

    for (i = numlist; --i >= 0; )
    {
        e = list[i];
        if (e == test_edge) continue;

        col1 = col2 = (long)(degree[e->prev->end]
                         + degree[e->next->end]) << 20;
        d1 = degree[e->start];
        d2 = degree[e->end];
        col1 += (long)d1 + ((long)d2 << 10);
        col2 += (long)d2 + ((long)d1 << 10);
        ...
        if (col1 == bestcol) good[num_good++] = e;
        if (col2 == bestcol) good[num_good++] = e->invers;
    }
   ...
   distinguished by oldnflip==0.  There are no loops yet. */
{
    ...
    if (minconnec == 2) got_one(nbtot,nbop,2+(numdoubles==0));

#ifdef PRE_FILTER_DOUBLE
    if (!(PRE_FILTER_DOUBLE)) return;
#endif
    ...
    if (minconnec == 1) scanordloops(nbtot,nbop,numdoubles);

    for (i = 0; i < nflips; ++i)
    {
        if (!feasible[i]) continue;

        a = flip[i]->start;     ama = am[a];
        b = flip[i]->end;       amb = am[b];
        c = flip[i]->prev->end; amc = am[c];
        d = flip[i]->next->end; amd = am[d];
        am[a] = ama & ~BIT(b);
        am[b] = amb & ~BIT(a);
        am[c] = amc | BIT(d);
        am[d] = amd | BIT(c);
        ...
        for (j = 0; j < numdoubles; ++j) if (doubles[j] == mate[i]) break;
        if (j == numdoubles)
        {
        ...
        if (ngood > 0
            && canon_edge(good,ngood,degree,numbering,&xnbtot,&xnbop))
   ...
   e->next->next->next->end, and FALSE otherwise. */
{
    ...
    for (e1 = ex->next; e1 != ex; e1 = e1->next)
        if (e1->end == u || e1->end == v) return FALSE;
...
/* initializes the datastructure neighbours_c4. This routine may only be called for
   triangulations!  Only for triangulations the set of neighbours is equal to the
   set of vertices a vertex shares a face with.  In fact it does the same as
   make_am applied to the neighbours_c4 array, but seems to be faster. */
{
  ...
  for (i=0; i<nv; i++)
    { run=firstedge[i];
      ...
      *bitv=BIT(run->end);
      for (j=degree[i]-1; j; j--) { run=run->next; *bitv |=BIT(run->end); }
      }
...
/* assumes that the graph it is called for is 4-connected and tests whether after
   removing the edge rem -- that bounds a triangle in prev-direction (left) --
   the graph is still 4-connected. If rem=a->b it is tested whether a vertex in
   the boundary of the face on the right of rem shares a face with a vertex 
   different from a,b that also shares a face with the unique vertex on the
   left of rem. 
...
/* 
   ...
   Assumes that the graph it is called for is 4-connected also after removing
   the edge rem.
   In prev direction of rem there must be a triangle. 

   It is assumed that the value in edge->left_facesize is correct.

   It computes the data for neighbours_c4 after the edge has been removed,
   but assumes that the edge has not yet been removed.
*/

{
  ...
  for ( ; j ; j--) 
    {  run=run->invers->next; face |= BIT(run->end); nb4_new[run->end] |= left_v;  }
...
#define REJECTC4 {++degree[v1]; ++degree[v2]; return 2;}
#define ORTESTC4(e) {col = DEGENDC4(e); \
 if (col > maxcol) REJECTC4 else if (col == maxcol) good_or[ng_or++] = e;}
#define INVTESTC4(e) {col = DEGENDC4(e); \
 if (col > maxcol) REJECTC4 else if (col == maxcol) good_inv[ng_inv++] = e;}
...
#define INVCOLC4(ed) \
 ((degree[ed->invers->next->end] << 10) + ed->invers->left_facesize)
    ...
    if (maxface > oldmaxface) oldmaxlist0 = oldmaxlist1;
 ...
 /* Now old edges that will be on a largest face (if *edel is deleted)
    ...
    v1 = edel->start;  v2 = edel->end;
    maxdeg = (degree[v1] > degree[v2] ? degree[v1] : degree[v2]) - 1;

    e1 = edel->prev;  v3 = e1->end;
    e4 = edel->next; v4 = e4->end;

    if (degree[v3] > maxdeg || degree[v4] > maxdeg) return 2;
    ...
    ng_or = nc_or;  ng_inv = nc_inv;

    if (maxface > 4)
    {
        if (degree[v2] == maxdeg) ORTESTC4(e3);
        if (degree[v1] == maxdeg) INVTESTC4(e4);
    }
 ...
 /* Now we have complete success!  Delete edel! */
    ...
    *ncan_inv = nc_inv;
    ...
    if (ng_or + ng_inv == 1) return 0;

    maxcol = 0;
    for (i = 0; i < nc_or; ++i)
        if (ORCOLC4(good_or[i]) > maxcol) maxcol = ORCOLC4(good_or[i]);
    for (i = 0; i < nc_inv; ++i)
        if (INVCOLC4(good_inv[i]) > maxcol) maxcol = INVCOLC4(good_inv[i]);

    for (i = 0, k = 0; i < nc_or; ++i)
        if (ORCOLC4(good_or[i]) == maxcol) good_or[k++] = good_or[i];
    *ncan_or = k;
    for (; i < ng_or; ++i)
    {
    ...
    *ngood_or = k;
    ...
    for (i = 0, k = 0; i < nc_inv; ++i)
        if (INVCOLC4(good_inv[i]) == maxcol) good_inv[k++] = good_inv[i];
    *ncan_inv = k;
    for (; i < ng_inv; ++i)
    {
...

{
    ...
    if (ne <= edgebound[1]) got_one(nbtot,nbop,3);
    if (ne == edgebound[0]) return;
    if (ne - 2*noldfeas > edgebound[1]) return;

#ifdef PRE_FILTER_POLY
    if (!(PRE_FILTER_POLY)) return;
#endif
    ...
    for (i = 0; i < noldfeas; ++i)
    {
        ...
        e = oldfeas[i];
        if (e->left_facesize != 3) e = e->invers;
        if (e->invers->left_facesize < oldmaxface-1
                      || e->invers->left_facesize == maxfacesize)
            continue;
...
#define REJECTC3 {++degree[v1]; ++degree[v2]; return 2;}
#define ORTESTC3(e) {col = DEGENDC3(e); \
 if (col > maxcol) REJECTC3 else if (col == maxcol) good_or[ng_or++] = e;}
#define INVTESTC3(e) {col = DEGENDC3(e); \
 if (col > maxcol) REJECTC3 else if (col == maxcol) good_inv[ng_inv++] = e;}
...
#define INVCOLC3(ed) \
 ...
 /* Now old edges that will be on a largest face (if *edel is deleted)
    ...
    if (maxface > 4)
    {
        if (degree[v2] == maxdeg) ORTESTC3(e3);
        if (degree[v1] == maxdeg) INVTESTC3(e4);
    }
 ...
 /* Now we have complete success!  Delete edel! */
    ...
    *ncan_inv = nc_inv;
    ...
    maxcol = 0;
    for (i = 0; i < nc_or; ++i)
        if (ORCOLC3(good_or[i]) > maxcol) maxcol = ORCOLC3(good_or[i]);
    for (i = 0; i < nc_inv; ++i)
        if (INVCOLC3(good_inv[i]) > maxcol) maxcol = INVCOLC3(good_inv[i]);

    for (i = 0, k = 0; i < nc_or; ++i)
        if (ORCOLC3(good_or[i]) == maxcol) good_or[k++] = good_or[i];
    *ncan_or = k;
    for (; i < ng_or; ++i)
    {
    ...
    *ngood_or = k;
    ...
    for (i = 0, k = 0; i < nc_inv; ++i)
        if (INVCOLC3(good_inv[i]) == maxcol) good_inv[k++] = good_inv[i];
    *ncan_inv = k;
    for (; i < ng_inv; ++i)
    {
...
#define REJECT(x) {++degree[v1]; ++degree[v2]; return x;}
#define ORTEST(e) {col = DEGEND(e); if (col > maxcol) \
  {if (OROK(e)) REJECT(2)} else if (col==maxcol&&OROK(e)) good_or[ng_or++]=e;}
#define INVTEST(e) {col = DEGEND(e); if (col > maxcol) \
  {if (INVOK(e)) REJECT(2)} \
   else if (col==maxcol&&INVOK(e)) good_inv[ng_inv++]=e;}
#define ORCOL(e) (((long)degree[e->invers->prev->end] << 10)  \
                      + e->left_facesize)
#define INVCOL(e) \
 ...
 /* mindeg and maxdeg are the degrees of the ends of edel after deletion */
    ...
    {
        mindeg = degree[v1]-1; maxdeg = degree[v2]-1;
    }
    else
    {
        mindeg = degree[v2]-1; maxdeg = degree[v1]-1;
    }
   ...
   /* The following is an efficiency short-cut, but it is assumed
    ...
    {
        if (ISNEQADJ(v3,e3->end) || ISNEQADJ(v3,v4)) return 2;
    }
    ...
    if (maxface == 4)
    {
     /* Recall from above that degree[v3,v4] <= maxdeg */
        ...
        if (ISNEQADJ(v3,v4))
        {
            ...
            if (degree[v3] == maxdeg)
            {
                if (col1 > maxcol) REJECT(2)
                else if (col1 == maxcol) good_inv[ng_inv++] = e1->invers;
                if (col2 > maxcol) REJECT(2)
                else if (col2 == maxcol) good_or[ng_or++] = e2->invers;
            }

            if (degree[v4] == maxdeg)
            {
                if (col1 > maxcol) REJECT(2)
                else if (col1 == maxcol) good_or[ng_or++] = e4->invers;
                if (col2 > maxcol) REJECT(2)
                else if (col2 == maxcol) good_inv[ng_inv++] = e3->invers;
            }
        }
    ...
    else
    {
        ...
        if (ISNEQADJ(v3,v5))
        {
            if (degree[v5] > maxdeg) REJECT(2);
            col = degree[v2];
            if (degree[v3] == maxdeg)
            {
                if (col > maxcol) REJECT(2)
                else if (col == maxcol) good_or[ng_or++] = e2->invers;
            }
            if (degree[v5] == maxdeg)
            {
                if (col > maxcol) REJECT(2)
                else if (col == maxcol) good_inv[ng_inv++] = e3->invers;
            }
        }
        if (ISNEQADJ(v3,v4))
        {
            if (degree[v4] > maxdeg) REJECT(2);
            col = degree[v1];
            if (degree[v3] == maxdeg)
            {
                if (col > maxcol) REJECT(2)
                else if (col == maxcol) good_inv[ng_inv++] = e1->invers;
            }
            if (degree[v4] == maxdeg)
            {
                if (col > maxcol) REJECT(2)
                else if (col == maxcol) good_or[ng_or++] = e4->invers;
            }
        }
        ...
        do
        {
            if (ISNEQADJ(ea->end,eb->end))
            {
                da = DEGEND(ea);
                db = DEGEND(eb);
                if (da > maxdeg || db > maxdeg) REJECT(2);
                col = degree[ea->start];
                if (da == maxdeg)
                {
                    if (col > maxcol) REJECT(2)
                    else if (col == maxcol) good_or[ng_or++] = ea->invers;
                }
                if (db == maxdeg)
                {
                    if (col > maxcol) REJECT(2)
                    else if (col == maxcol) good_inv[ng_inv++] = eb->invers;
                }
            }
            inmaxface[nml1++] = ea;
            ea = eb->invers;
 ...
 /* Now test old edges still on max faces */
    ...
    if (newconnec < minpolyconnec) REJECT(1);
 ...
 /* Now we have complete success!  Just prune the lists a bit and
    ...
    *ncan_inv = nc_inv;
    ...
    maxcol = 0;
    for (i = 0; i < nc_or; ++i)
        if (ORCOL(good_or[i]) > maxcol) maxcol = ORCOL(good_or[i]);
    for (i = 0; i < nc_inv; ++i)
        if (INVCOL(good_inv[i]) > maxcol) maxcol = INVCOL(good_inv[i]);

    for (i = 0, k = 0; i < nc_or; ++i)
        if (ORCOL(good_or[i]) == maxcol) good_or[k++] = good_or[i];
    *ncan_or = k;
    for (; i < ng_or; ++i)
    {
        col = ORCOL(good_or[i]);
    ...
    *ngood_or = ng_or = k;
    ...
    for (i = 0, k = 0; i < nc_inv; ++i)
        if (INVCOL(good_inv[i]) == maxcol) good_inv[k++] = good_inv[i];
    *ncan_inv = k;
    for (; i < ng_inv; ++i)
    {
   ...
   are given as 3. */
{
    ...
    if (ne <= edgebound[1]) got_one(nbtot,nbop,connec);
    if (ne == edgebound[0]) return;
    if (ne - 2*noldfeas > edgebound[1]) return;
...

{
   ...
   /* In the -m5 case, the group must be saved and restored. */
    ...
    for (i = 0; i < nv; ++i)
    {
        ...
        do
        {
            if (e == e->min && degree[e->start] > minpolydeg
                            && degree[e->end] > minpolydeg)
                feasible[nfeas++] = e;
            e->left_facesize = 3;
            am2[e->start][e->end] = 1;
            e = e->next;
   ...
   Returns TRUE if 3-connected, FALSE otherwise. */
{
    ...
    if (nv == 3) return FALSE;   /* K3 has connectivity 2 */
   ...
   removing inequivalent vertices of the required degree. */
{
    ...
    for (v = 0; v < nv; ++v)
        if (polygonsize <= 0 || degree[v] == polygonsize) vmark[v] = 1;
        else                                              vmark[v] = 0;
   ...
   values returned by canon() for that graph. */
{
    ...
    if (nv == maxnv)
    {
        if (pswitch)               startpolyscan(nbtot,nbop);
        else if (polygonsize >= 0) polygon_triang(nbtot,nbop);
        else if (minconnec == 3)   got_one(nbtot,nbop,3);
        else                       scandouble(nbtot,nbop,0,NULL,NULL,0,NULL);
        return;
    }
...
#endif 
        {
            if (canon(degree,numbering,&xnbtot,&xnbop))    
            {
                ...
                if (xnbtot == 1)
                {
                    ...
                    {
                        if (xnbop == 1)
                            if (valid5edge(e2->prev)) e2 = e2->prev;
                            else                      e2 = e2->next;
                        else
...
*/
{
    ...
    start=P_op(nv);
    rp=ref->prev; rn=ref->next;

    firstedge[ref->start]=rn; /* just in case ref was "firstedge" */
    ref->start=ref->invers->end=nv; ref->next=start+4; ref->prev=start+2;
    degree[nv]=degree[nv+1]=4;
    firstedge[nv]=start; firstedge[nv+1]=start+1;


    buffer=rn->end;
    degree[buffer]+=2;
    work=start+4; work->end=buffer; work->prev=ref;
    work2=rn->invers->next;
    /*work=start+5;*/ work++;
      work->start=buffer; work->next=work2; work2->prev=work;
    work2=rn->invers;
    /*work=start+6;*/ work++;
      work->start=buffer; work->prev=work2; work2->next=work;
    /*(start+7)*/ work++; work->end=buffer;

    buffer=rn->start;
    /*work=start+8;*/ work++;
      work->start=buffer; work->prev=rp; work->next=rn; rn->prev=rp->next=work;
    /*(start+9)*/ work++; work->end=buffer;

    buffer=rp->end;
    degree[buffer]+=2;
    work2=rp->invers->prev;
    /*work=start+10;*/ work++;
      work->start=buffer; work->next=rp->invers; rp->invers->prev=work;
    /*(start+11)*/ work++; work->end=buffer;
    work=start+3; work->start=buffer; work->prev=work2; work2->next=work; 
    /*work=start+2;*/ work--; work->end=buffer; work->next=ref; 
...
*/
{
    ...
    degree[a->end]-=2;  degree[b->end]-=2;

    ref->start=ref->invers->end=c->start;
    firstedge[ref->start]=ref;
    ref->next=d; d->prev=ref;
    ref->prev=c; c->next=ref;

    a=a->invers; c=c->invers;
    a->next=c; c->prev=a; firstedge[a->start]=a;

    b=b->invers; d=d->invers;
    b->prev=d; d->next=b; firstedge[b->start]=b;
...
*/
{
    ...
    if (hint)
    {
        ...
        w = e->end;
        e1 = e->next->next;
        if (degree[w] >= degend && degree[e->start] == 4
                  && degree[e->next->end] != 4 && degree[e->prev->end] != 4
                  && degree[e1->end] == 4 && e != ref)
        {
    ...
    if (degree[i] == 4)
    {
        e = firstedge[i];
        j = 0;
        if (degree[e->end] == 4) {e1 = e; ++j;} e=e->next;
        if (degree[e->end] == 4) {e1 = e; ++j;} e=e->next;
        if (degree[e->end] == 4) {e1 = e; ++j;} e=e->next;
        if (degree[e->end] == 4) {e1 = e; ++j;}
        if (j != 1) continue;

        e = e1->next->next;
        
        w = e->end;
        if (degree[w] < degend) continue;
        if (e == ref) continue;
        z = e1->invers->next->next->end;
   ...
   otherwise 0.  It must be that e is an edge of the graph. */
{
    ...
    if (degree[e->start] != 4 || degree[e->end] == 4) return 0;
    if (degree[e->next->end] == 4 || degree[e->prev->end] == 4) return 0;
    e1 = e->next->next;
    if (degree[e1->end] != 4) return 0;
    z = e1->invers->next->next->end;
    if (degree[z] == 4) return 1;
    ew = e->invers;
    ewlast = ew->prev;
    for (ew = ew->next->next; ew != ewlast; ew = ew->next)
        if (ew->end == z) break;
    if (ew != ewlast) return 0;
    else              return 1;
}
...
*/
{
    ...
    dummy=c->start; dummy2=a->start;
    (degree[dummy])+=2; (degree[dummy2])+=2;
    degree[nv]=4; (degree[b->end])-=2;
    start->prev=x; x->next=start; start->end=dummy; start++;
    /*1*/ start->next=c; c->prev=start; start->start=dummy; start++;
    /*2*/ start->prev=d; d->next=start; start->start=dummy;
          start->end=dummy2; start++;
    /*3*/ start->next=b; b->prev=start; start->start=dummy2;
          start->end=dummy; start++;
    /*4*/ start->next=ref; ref->prev=start; start->end=dummy2; start++;
    /*5*/ start->prev=a; a->next=start; start->start=dummy2;
...
*/
{
    ...
    (degree[b->end])+=2;
    (degree[a->start])-=2; (degree[c->start])-=2; 
...
*/
{
    ...
    besttype = ((long)degree[ref->end]<<10)
               + degree[ref->next->end] + degree[ref->prev->end];
    good_or[0] = ref;
    *ngood_or = *ngood_ref = 1;

    e = ref->next;
    thistype = ((long)degree[e->end]<<10)
               + degree[e->next->end] + degree[e->prev->end];
    if (thistype >= besttype)
    {
        good_mir[0] = e;
        *ngood_mir = *ngood_mir_ref = 1;
    ...
    if (degree[i] == 4)
    {
        ...
        {
            if (degree[e->prev->end] < 6 || degree[e1->end] < 6) continue;
            if (e == ref) continue;
            ...
            do
            {
                if (ew->end == z1 || ew->end == z2) break;
                ew = ew->next;
            } while (ew != ewlast);
            if (ew->end == z1 || ew->end == z2) continue;

            thistype = ((long)degree[z1]<<10)
                                + degree[z2] + degree[e->prev->end];
            if (thistype > besttype)
            {
                *ngood_ref = *ngood_mir_ref = 0;
                return;
...
*/
{
    ...
    if (i == nPedges-1 || is_bip_P(Pedges[i]))
    {
        ...
        ++weight[e->start]; ++weight[e->end];
        while (degree[e->end] == 4) e = e->invers->next->next;
        MARKLO(e);
        ++weight[e->start];
        ...
        else             /* Case of nontrivial group */
        {
            ...
            for (i = 0; i < ne; ++i, ++nb0)
            {
                ...
                if (!ISMARKEDLO(e) && degree[e->start] >= 6)
                {
                    ...
                    nb = nb0 + MAXE;
                    for (; nb < nboplim; nb += MAXE) MARKLO(*nb);
                    for (; nb < nblim; nb += MAXE) MARKLO((*nb)->prev);
                }
            }
        }
    }
...
#endif 
        {
            if (!hint || !is_bip_P(hint)) hint = has_bip_P();
...
/* Make new list of non-facial triangles after a Q-extension. */
{
  ...
  /* Remove triangles that include ref or ref->next */
    k = 0;
    for (i = 0; i < numnft; ++i)
        if (nft[i].e1 != refmin1 && nft[i].e2 != refmin1
                && nft[i].e3 != refmin1 && nft[i].e1 != refmin2
                && nft[i].e2 != refmin2 && nft[i].e3 != refmin2)
            newnft[k++] = nft[i];
   ...
   hit all of nft[0..numnft-1]. */
{
    ...
    {
        RESETMARKS;
        MARKLO(nft[j].e1); MARKLO(nft[j].e1->invers);
        MARKLO(nft[j].e2); MARKLO(nft[j].e2->invers);
        MARKLO(nft[j].e3); MARKLO(nft[j].e3->invers);
        
        k = 0;
        for (i = 0; i < *nextP; ++i)
            if (ISMARKEDLO(extP[i])) extP[k++] = extP[i];
        *nextP = k;
    }
}
...
*/
{
    ...
    {
        connec = 3 + (numnft==0);
        if (connec >= minconnec) got_one(nbtot,nbop,connec);
        return;
    }
...
#endif 
        {
            ...
            if (ngood_ref+ngood_mir_ref > 0)
            {
                                                ...
                                                && ngood_mir == ngood_mir_ref)
                {
                    ...
                    if (connec >= minconnec) got_one(1,1,connec);
                }
                else if (ngood_or+ngood_mir==1)
                {
                    update_nft_P(nft,numnft,extP[i],newnft,&newnumnft);
...
*/
{
    ...
    degree[nv]=4; firstedge[nv]=start;

    dummy=start+1;
    start->end=dummy->start=a->start;
    (degree[a->start])++;
    a->prev=b->next=dummy;
    dummy->prev=b; dummy->next=a;

    b=b->invers; a=ref->prev;

    start+=2; /*2*/ dummy=start+1;
    start->end=dummy->start=b->start;
    firstedge[b->start]=dummy;
    b->prev=a->next=dummy;
    dummy->prev=a; dummy->next=b;

    a=a->invers; b=a->prev;
    start+=2; /*4*/ dummy=start+1;
    start->end=dummy->start=b->start;
    (degree[b->start])++;
    a->prev=b->next=dummy;
    dummy->prev=b; dummy->next=a;
    ...
    ref->next=start-4; ref->prev=start;
...
*/
{
    ...
    if (degree[ref->start] != 4) return 0;
    if (degree[ref->prev->end] == 4 || degree[ref->next->end] == 4) return 0;

    w = ref->next->next->end;
    if (degree[w] == 4) return 1;
    e = ref->invers;
    elast = e->prev;
...
/* The reverse of extend_four(ref) */
{
    ...
    a->prev=b; b->next=a; 
    firstedge[a->start]=a; (degree[a->start])--;

    a=b->invers; b=a->prev->prev;
    a->prev=b->next=ref;
    ref->next=a; ref->prev=b;
...
#define FOURTYPE(d3,d4) (((d3)<<10)+(d4))
    ...
    if (deg1 <= deg2)
    {
        bestdeg = deg2; nextdeg = deg1;
        ertype1 = FOURTYPE(deg4,deg3);
        ertype2 = FOURTYPE(deg3,deg4);
        besttype = ertype = ertype1 > ertype2 ? ertype1 : ertype2;
    }

    if (deg1 == deg2) besttype = etype > ertype ? etype : ertype;

    if (deg1 >= deg2)
    {
        if (etype1 == besttype) good_or[nor++] = e;
        if (etype2 == besttype) good_mir[nmir++] = e;
    }
    if (deg1 <= deg2)
    {
        if (ertype1 == besttype) good_or[nor++] = er;
        if (ertype2 == besttype) good_mir[nmir++] = er;
    }
    ...
    else if (degree[i] == bestdeg)
    {
        ...
        do
        {
            if (degree[ei->end] == 4 && !ISMARKEDLO(ei))
            {
                ...
                else if (deg2 == nextdeg && (bestdeg > nextdeg || i < w)
                                         && is_min4_four(e))
                {
                    deg3 = degree[e->prev->end];
                    deg4 = degree[e->next->end];
   ...
   in Batagelj's paper. */
{
    ...
    start=five_op(nv);
    degree[nv]=5; firstedge[nv]=start;
    ...
    a=b->invers; b=a->prev->prev->prev; dummy=ref->invers;
    b->next=dummy; 
    dummy->prev=b;
    firstedge[a->start]=a;
    (degree[a->start])--;
    ...
    dummy->next=start-4; dummy->prev=start;
    start->next=dummy; (start-4)->prev=dummy;

    ref->start=ref->invers->end=nv;
    ref->next=start; ref->prev=start-2;
    start->prev=ref; (start-2)->next=ref;
...
/* The reverse of extend_five(). */
{
    ...
    a=ref->next->next; dummy=ref->invers; b=dummy->prev;
    dummy->prev=b->next=a; a->next=dummy; a->prev=b;
    ...
    a=b->invers->prev->prev->invers; b=a->prev->prev;
    a->prev=ref; ref->next=a; b->next=ref; ref->prev=b;
    ref->start=ref->invers->end=a->start;
    firstedge[a->start]=a;
...
#define FIVETYPE(d1,d2,d3) (((d1)<<21)+((d2)<<11)+(d3))
    ...
    nor = nmir = 0;
    if (type1 == besttype) good_or[nor++] = e;
    if (type2 == besttype) good_mir[nmir++] = e;
    ...
    if (degree[i] == 5)
    {
        for (e = firstedge[i], j = 0; j < 5; ++j, e = e->next)
        {
            ...
            for (ez = ez->next->next; ez != ezlast; ez = ez->next)
                if (ez->end == w || ez->end == v) break;
            if (ez != ezlast) continue;
...

{
    ...
    ref->next=start+10; (start+10)->prev=ref;
    a->next=start+14; (start+14)->prev=a;
    b->next=start+6; (start+6)->prev=b;

    ref=ref->invers;
    ref->prev=start+12; (start+12)->next=ref;
    a=a->invers;
    a->prev=start+16; (start+16)->next=a;
    b=b->invers;
    b->prev=start+8; (start+8)->next=b;
...
*/
{
    ...
    ref->next=b->invers; b->invers->prev=ref;
    a->next=ref->invers; ref->invers->prev=a;
    b->next=a->invers; a->invers->prev=b;
    ...
    if (degree[i] >= 6)
    {
        for (e = firstedge[i], j = degree[i]; --j >= 0; e = e->next)
        {
            ...
            if (degree[e->end] < 6 || degree[e->next->end] != 4 ||
                ...
                degree[e->next->next->next->end] < 6) continue;
            if (degree[e->next->invers->next->next->end] != 4) continue;
    ...
    if (known) 
    {
        ...
        if (deg3 == 0)
        {
          /* edges around the outside */
            e = known->invers->next;
            ext4[k++] = e; MARKLO(e); MARKLO(e->invers);
      ...
      /* S-extensions, non-trivial group */
        ...
        if (nv <= maxnv-3)
        {
            ...
            for (i = 0; i < ne; ++i)
            {
                ...
                for (nb = nb0 + i; nb < nboplim; nb += MAXE) MARKLO(*nb);

                for (; nb < nblim; nb += MAXE) MARKLO((*nb)->invers);
...
/* Make new list of non-facial triangles after a five-extension. */
{
  ...
  /* Remove triangles that include either of the former two chords */
    ...
    for (i = 0; i < numnft; ++i)
        if (nft[i].e1 != refmin && nft[i].e2 != refmin && nft[i].e3 != refmin
                && nft[i].e1 != amin && nft[i].e2 != amin && nft[i].e3 != amin)
            newnft[k++] = nft[i];
   ...
   for which not all of nft[0..numnft-1] hit some chord. */
{
    ...
    for (j = 0; j < numnft; ++j)
    {
        ...
        k = 0;
        for (i = 0; i < *next4; ++i)
            if (ISMARKEDLO(ext4[i])) ext4[k++] = ext4[i];
        *next4 = k;
...
#define FIND_EXTENSIONS_MIN4 find_extensions_min4
    ...
    if (nv == maxnv-1 && numnft > 0 && minconnec == 4) /* rules out m4c3x */
...

{
  ...
  counter=1; run=e3; run->start=run->invers->end=nv;
  do { run=run->next; run->start=run->invers->end=nv; counter++; }
  while (run != e4);
...

{
  ...
  counter=1; run=e3; run->start=run->invers->end=center;
  do { run=run->next; run->start=run->invers->end=center; counter++; }
  while (run != e4);
  ...
  e1->next=e3; e3->prev=e1;
  e2->prev=e4; e4->next=e2;

  e1i->prev=work1; work1->next=e1i;
  e2i->next=work2; work2->prev=e2i;
...
if (!do_mirror)
  ...
  start->start=end2;
  start->prev=e4; e4->next=start;
  start->next=e2i; e2i->prev=start;
  (degree[end2])++;

  run=start+1; 
  run->end=end2; run->next=work1; work1->prev=run;

  run++; /*2*/
  run->start=(run+1)->end=center1; 
  run->prev=e2; e2->next=run;
  run->next=ei; ei->prev=run;

  run+=2; /*4*/
  run->start=(run+1)->end=center2;
  run->prev=e; e->next=run;

  run+=2; /*6*/
  run->next=work2; work2->prev=run;

  run++; /*7*/ 
  run->prev=work1; work1->next=run;

  run++; /*8*/
  run->start=(run+1)->end=center2;
  run->next=e3i; e3i->prev=run;

  run+=2; /*10*/
  run->start=start3;
  run->next=e5i; e5i->prev=run;
  run->prev=e3; e3->next=run;

  run++; /*11*/
  run->end=start3;
  run->prev=work3; work3->next=run;
  degree[start3]++;
  ...
  start->start=end2;
  start->next=e4; e4->prev=start;
  start->prev=e2i; e2i->next=start;
  (degree[end2])++;

  run=start+1; 
  run->end=end2; run->prev=work1; work1->next=run;

  run++; /*2*/
  run->start=(run+1)->end=center1; 
  run->next=e2; e2->prev=run;
  run->prev=ei; ei->next=run;

  run+=2; /*4*/
  run->start=(run+1)->end=center2;
  run->next=e; e->prev=run;

  run+=2; /*6*/
  run->prev=work2; work2->next=run;

  run++; /*7*/ 
  run->next=work1; work1->prev=run;

  run++; /*8*/
  run->start=(run+1)->end=center2;
  run->prev=e3i; e3i->next=run;

  run+=2; /*10*/
  run->start=start3;
  run->prev=e5i; e5i->next=run;
  run->next=e3; e3->prev=run;

  run++; /*11*/
  run->end=start3;
  run->next=work3; work3->prev=run;
  degree[start3]++;
...
if (!do_mirror)
   {
     ...
     e4->next=e2i; e2i->prev=e4;
     e2->next=work1; work1->prev=e2;
     ei->prev=work1; work1->next=ei;
     e->next=work2; work2->prev=e;
     e3i->prev=work3; work3->next=e3i;
     e3->next=e5i; e5i->prev=e3;
...
else
   {
     ...
     e4->prev=e2i; e2i->next=e4;
     e2->prev=work1; work1->next=e2;
     ei->next=work1; work1->prev=ei;
     e->prev=work2; work2->next=e;
     e3i->next=work3; work3->prev=e3i;
     e3->prev=e5i; e5i->next=e3;
...
/* Make an icosahedron using the first 60 edges */
{
   ...
   EDGE *buffer;
    ...
    for (i=0; i<=11; i++)
      { buffer=edges+5*i;
        firstedge[i]=buffer; degree[i]=5;
        buffer->next=buffer+1; buffer->prev=buffer+4; buffer->start=i;
        buffer++; buffer->next=buffer+1; buffer->prev=buffer-1; buffer->start=i;
        buffer++; buffer->next=buffer+1; buffer->prev=buffer-1; buffer->start=i;
        buffer++; buffer->next=buffer+1; buffer->prev=buffer-1; buffer->start=i;
        buffer++; buffer->next=buffer-4; buffer->prev=buffer-1; buffer->start=i;
      }
...
*/
    ...
    for (n=12; n<MAXN; n++)
      { run=start=min5_a(n);
        ...
        run->next=start+1; run->prev=start+5;

        run=start+4; run->end=n; run->min=run; run->invers=start+5;

        run=start+5; run->start=n; run->min=run->invers=start+4;
        run->next=start+3;
      }
...
*/
    ...
    for (n=12; n<MAXN-1; n++)
      { run=start=min5_b0(n);
        ...
        run=start+2; run->end=n;
        run->min=run; run->invers=start+3 ; 

        run=start+3; run->start=n;
        run->min=run->invers=start+2; run->prev=start+5; run->next=start+1;

        run=start+4; run->end=n;
        run->min=run; run->invers=start+5; run->next=start+8;

        run=start+5; run->start=n; 
        run->min=run->invers=start+4; run->prev=start+7; run->next=start+3;

        run=start+6; run->start=n+1; run->end=n;
        run->min=run; run->invers=start+7; run->prev=start+9;

        run=start+7; run->start=n; run->end=n+1;
        run->min=run->invers=start+6; run->next=start+5;

        run=start+8; run->end=n+1;
        run->min=run; run->invers=start+9; run->prev=start+4; 

        run=start+9; run->start=n+1; 
        run->min=run->invers=start+8; run->prev=start+11; run->next=start+6;

        run=start+10; run->end=n+1;
        run->min=run; run->invers=start+11; 

        run=start+11; run->start=n+1;
        run->min=run->invers=start+10; run->next=start+9;
    ...
    for (n=12; n<MAXN-1; n++)
      { run=start=min5_b1(n);
        ...
        run=start+1; run->start=n; 
        run->min=run->invers=start; run->next=start+3; 

        run=start+2; run->end=n;
        run->min=run; run->invers=start+3 ; 

        run=start+3; run->start=n;
        run->min=run->invers=start+2; run->next=start+5; run->prev=start+1;

        run=start+4; run->end=n;
        run->min=run; run->invers=start+5; run->prev=start+8;

        run=start+5; run->start=n; 
        run->min=run->invers=start+4; run->next=start+7; run->prev=start+3;

        run=start+6; run->start=n+1; run->end=n;
        run->min=run; run->invers=start+7; run->next=start+9;

        run=start+7; run->start=n; run->end=n+1;
        run->min=run->invers=start+6; run->prev=start+5;

        run=start+8; run->end=n+1;
        run->min=run; run->invers=start+9; run->next=start+4; 

        run=start+9; run->start=n+1; 
        run->min=run->invers=start+8; run->next=start+11; run->prev=start+6;

        run=start+10; run->end=n+1;
        run->min=run; run->invers=start+11; 

        run=start+11; run->start=n+1;
        run->min=run->invers=start+10; run->prev=start+9;
   ...
   has its start at an old vertex and an edge of the pentagon in next direction. */
    ...
    for (n=12; n<MAXN-5; n++)
      { 
        ...
        for (i=0; i<4; i++)
          { run=start+10+5*i;
            run->start=n+i; run->prev=run+4; run->next=run+1;
            run++; run->start=n+i; run->prev=run-1; run->next=run+1;
            run++; run->start=n+i; run->prev=run-1; run->next=run+1;
            run++; run->start=n+i; run->prev=run-1; run->next=run+1;
            run++; run->start=n+i; run->prev=run-1; run->next=run-4;
          }
        run=start+10+5*i;
        run->prev=run+4; run->next=run+1;
        run++; run->start=n+i; run->prev=run-1; run->next=run+1;
        run++; run->start=n+i; run->prev=run-1; run->next=run+1;
        run++; run->start=n+i; run->prev=run-1; run->next=run+1;
        run++; run->start=n+i; run->prev=run-1; run->next=run-4;
        i++;
        run=start+10+5*i;
        run->start=n+i-1; run->prev=run+4; run->next=run+1;
        run++; run->start=n+i-1; run->prev=run-1; run->next=run+1;
        run++; run->start=n+i-1; run->prev=run-1; run->next=run+1;
        run++; run->start=n+i-1; run->prev=run-1; run->next=run+1;
        run++; run->start=n+i-1; run->prev=run-1; run->next=run-4;
        ...
        run=start;
        run->prev=run+1; (run+1)->next=run;
        run+=2; run->prev=run+1; (run+1)->next=run;
        run+=2; run->prev=run+1; (run+1)->next=run;
        run+=2; run->prev=run+1; (run+1)->next=run;
        run+=2; run->prev=run+1; (run+1)->next=run;
        ...
        run=start;    run->end=n;   run->min=start; run->invers=start+10;
        run++; /*1*/  run->end=n+1; run->min=run; run->invers=start+15;
        run++; /*2*/  run->end=n+1; run->min=run; run->invers=start+16;
        run++; /*3*/  run->end=n+2; run->min=run; run->invers=start+21;
        run++; /*4*/  run->end=n+2; run->min=run; run->invers=start+22;
        run++; /*5*/  run->end=n+3; run->min=run; run->invers=start+26;
        run++; /*6*/  run->end=n+3; run->min=run; run->invers=start+27;
        run++; /*7*/                run->min=run; run->invers=start+31;
        run++; /*8*/                run->min=run; run->invers=start+32;
        run++; /*9*/  run->end=n;   run->min=run; run->invers=start+14;
        run++; /*10*/               run->min=run->invers=start;
        run++; /*11*/ run->end=n+1; run->min=run; run->invers=start+19;
        run++; /*12*/ run->end=n+4; run->min=run; run->invers=start+35;
        run++; /*13*/               run->min=run; run->invers=start+33;
        run++; /*14*/               run->min=run->invers=start+9;
        run++; /*15*/               run->min=run->invers=start+1;
        run++; /*16*/               run->min=run->invers=start+2;
        run++; /*17*/ run->end=n+2; run->min=run; run->invers=start+20;
        run++; /*18*/ run->end=n+4; run->min=run; run->invers=start+36;
        run++; /*19*/ run->end=n;   run->min=run->invers=start+11;
        run++; /*20*/ run->end=n+1; run->min=run->invers=start+17;
        run++; /*21*/               run->min=run->invers=start+3;
        run++; /*22*/               run->min=run->invers=start+4;
        run++; /*23*/ run->end=n+3; run->min=run; run->invers=start+25;
        run++; /*24*/ run->end=n+4; run->min=run; run->invers=start+37;
        run++; /*25*/ run->end=n+2; run->min=run->invers=start+23;
        run++; /*26*/               run->min=run->invers=start+5;
        run++; /*27*/               run->min=run->invers=start+6;
        run++; /*28*/               run->min=run; run->invers=start+30;
        run++; /*29*/ run->end=n+4; run->min=run; run->invers=start+38;
        run++; /*30*/ run->end=n+3; run->min=run->invers=start+28;
        run++; /*31*/               run->min=run->invers=start+7;
        run++; /*32*/               run->min=run->invers=start+8;
        run++; /*33*/ run->end=n;   run->min=run->invers=start+13;
        run++; /*34*/ run->end=n+4; run->min=run; run->invers=start+39;
        run++; /*35*/ run->end=n;   run->min=run->invers=start+12;
        run++; /*36*/ run->end=n+1; run->min=run->invers=start+18;
        run++; /*37*/ run->end=n+2; run->min=run->invers=start+24;
        run++; /*38*/ run->end=n+3; run->min=run->invers=start+29;
        run++; /*39*/               run->min=run->invers=start+34;
...
*/
  ...
  *anchor=e;
  ...
  firstedge[nv]=start+10; degree[nv]=5; nv++;
  firstedge[nv]=start+16; degree[nv]=5; nv++;
  firstedge[nv]=start+23; degree[nv]=5; nv++;
  firstedge[nv]=start+28; degree[nv]=5; nv++;
  firstedge[nv]=start+35; degree[nv]=5; nv++;
  firstedge[origin]=start+32; 
  /* degree[nv]=5; is already the case */

  ne+=30;

  e1->prev=start; start->next=e1;
  run=start+1;
  e5i->next=run; run->prev=e5i; 
  run++;
  e5->prev=run; run->next=e5;
  run++;
  e4i->next=run; run->prev=e4i;
  run++;
  e4->prev=run; run->next=e4;
  run++;
  e3i->next=run; run->prev=e3i;
  run++;
  e3->prev=run; run->next=e3;
  run++;
  e2i->next=run; run->prev=e2i;
  run++;
  e2->prev=run; run->next=e2;
  run++;
  e1i->next=run; run->prev=e1i;
...

{
  ...
  if ((degree[e->next->end]==5) || (degree[e->prev->end]==5)) return TRUE;
  ...
  run=e->next->next;
    ...
    run1=run1->next;
    MARK_V(run1->end);
    while (run1!=last1) { run1=run1->next; MARK_V(run1->end); }
  run=run->next;
    dummy=run->invers;
    last1=dummy->prev->prev;
    run1=dummy->next->next;
  ...
  run=e->next->next;
    ...
    last1=dummy->prev->prev;
    run1=dummy->next->next;
    if (ISMARKED_V(run1->end)) return TRUE;
    run1=run1->next;
    if (ISMARKED_V(run1->end)) return TRUE;
    while (run1!=last1) { run1=run1->next; if (ISMARKED_V(run1->end)) return TRUE; }
  run=run->next;
    dummy=run->invers;
    last1=dummy->prev->prev;
    run1=dummy->next->next;
...
*/
{
    ...
    if (nbtot == 1)
    {
            ...
            else
            {
                ...
                do
                {
                    ...
                    do
                    {
                        if (e2 > e1)
                        {
                            if (e1->end == v1 || e2->end == v1) xd1 = d1+1;
                            else                                xd1 = d1;
                            if (e1->end == v2 || e2->end == v2) xd2 = d2+1;
                            else                                xd2 = d2;
      ...
      /* C-extensions, trivial group */
        ...
        else
        {
            k = 0;
            for (i = 0; i < nv; ++i)
                if (degree[i] == 5) extC[k++] = firstedge[i];
            *nextC = k;
        }
    }
    else    
    {
      ...
      /* A-extensions, non-trivial group */
        ...
        if (degree[i] >= 6)
        {
            if (!ISMARKED_V(i) && lastA && degree[i]+4 < d1+d2) continue;
      ...
      /* B-extensions, non-trivial group */
        ...
        else
        {
            ...
            for (i = 0; i < ne; ++i)
                if (!ISMARKEDLO(nb0[i]) && degree[nb0[i]->start] == 5
                                         && degree[nb0[i]->end] == 5)
                {
                    extB[k] = nb0[i]->min;
                    if (nb0[i] == nb0[i]->min)
...
#define UPAIR(x,y) ((x)<(y) ? PAIR(x,y) : PAIR(y,x))
    ...
    for (i = nprevA; --i >= 0;)
    {
        ...
        MARKLO(hint);
        MARKLO(hint->invers);
        if ((d3 = degree[hint->prev->end]) >= 6
                      && (d4 = degree[hint->next->end]) >= 6)
        {
            ...
            if (col1 == bestcol1)
            {
                ...
                {   
                    if (nor + nmir == 0)
                    {
                        if (deg1 >= deg2)
                        {
                            if (deg3 >= deg4) good_or[nor++] = e;
                            if (deg3 <= deg4) good_mir[nmir++] = e;
                        }
                        if (deg1 <= deg2)
                        {
                            if (deg3 <= deg4) good_or[nor++] = e->invers;
                            if (deg3 >= deg4) good_mir[nmir++] = e->invers;
                        }
                        *ngood_ref = nor;
                        *ngood_mir_ref = nmir;
                    }
                    ...
                    if (d1 >= d2)
                    {   
                        if (d3 >= d4) good_or[nor++] = hint;
                        if (d3 <= d4) good_mir[nmir++] = hint;
                    }
                    if (d1 <= d2)
                    {
                        if (d3 <= d4) good_or[nor++] = hint->invers;
                        if (d3 >= d4) good_mir[nmir++] = hint->invers;
                    }
                }
            }
    ...
    if (*ngood_ref+*ngood_mir_ref == 0) return;
    ...
    {
        if (d1 <= maxdeg12) mind2 = sumdeg12 - d1;
        else                mind2 = sumdeg12 - d1 + 1;

        ei = eilast = firstedge[i];
        do
        {
            if (!ISMARKEDLO(ei) && (d2 = degree[ei->end]) >= mind2
                                && (d3 = degree[ei->prev->end]) >= 6
                                && (d4 = degree[ei->next->end]) >= 6)
            {
                col1 = UPAIR(d1,d2);
                ...
                else
                {
                    ...
                    else if (col2 == bestcol2 && !on_nf_5_cycle(ei))
                    {
                        if (d1 >= d2)
                        {
                            if (d3 >= d4) good_or[nor++] = ei;
                            if (d3 <= d4) good_mir[nmir++] = ei;
                        }
                        if (d1 <= d2)
                        {
                            if (d3 <= d4) good_or[nor++] = ei->invers;
                            if (d3 >= d4) good_mir[nmir++] = ei->invers;
                        }
                    }
                }
            }
...

{
    ...
    if (!mirror)
    {
        if (degree[ref->next->next->end] < 6
                                         || degree[refi->next->next->end] < 6)
            return FALSE;
        ...
        for (e = e2->next->next->next->next; e != e2; e = e->next)
            if (ISMARKED_V(e->end)) return FALSE;
        e2 = e2->invers;
    ...
    {
        if (degree[ref->prev->prev->end] < 6
                                         || degree[refi->prev->prev->end] < 6)
            return FALSE;
...

{
    ...
    for (i = 0; i < nv; ++i)
    {
        ...
        do
        {
            if (e == e->min && degree[e->next->end] >= 6
                    && degree[e->prev->end] >= 6 && !on_nf_5_cycle(e))
                return TRUE;
            e = e->next;
        } while (e != elast);
    }
...

{
    ...
    if (degree[i] == 5)
    {
        ...
        do  
        {
            if (e == e->min && degree[e->end] == 5
                    && degree[e->prev->end] == 5
                    && degree[e->next->end] == 5
                    && (is_valid_min5_b(e,FALSE) || is_valid_min5_b(e,TRUE)))
                return TRUE;
            e = e->next;
        } while (e != elast);
    }
...
*/
{
    ...
    *ngood_mir_ref = nmir;
    ...
    if (degree[i] == 5)
    {
        ...
        do
        {
            if (e == e->min && !ISMARKEDLO(e) && degree[e->prev->end] == 5
                        && degree[e->end] == 5 && degree[e->next->end] == 5)
            {
 ...
 
{
  ...
  if (degree[v]!=5) return FALSE;
 
  run=firstedge[v]; if(degree[run->end]!=5) return FALSE;
  run=run->next; if(degree[run->end]!=5) return FALSE;
  run=run->next; if(degree[run->end]!=5) return FALSE;
  run=run->next; if(degree[run->end]!=5) return FALSE;
  run=run->next; if(degree[run->end]!=5) return FALSE;
 
  run=run->invers->prev->prev->invers->next; /* edge 1*/
  if ((degree[run->start]<6) || (degree[run->end]<6)) return FALSE;
  run=run->invers->next->next->next; /* edge 2*/
  if (degree[run->end]<6) return FALSE;
...
#define FIND_EXTENSIONS_MIN5 find_extensions_min5
    ...
    for (i = 0; i < nextC; ++i)
    {
...
#endif  
        {
            ...
            {
                for (j = 0; j < nv-1; ++j)
                    if (is_min5_c_centre(j)) colour[j] = 2;
                    else                     colour[j] = degree[j];
                colour[nv-1] = 2;
 ...
 
{
  ...
  run=e->next;
  last=e->prev->prev;
  if (ISMARKED_V(run->end)) return 0;
  while (run!=last)
    { run=run->next;
      if (ISMARKED_V(run->end)) return 0;
    }
 ...
 
{
  ...
  run=e->next;
  last=e->prev->prev;
  if (ISMARKED_V(run->end)) return 1;
  while (run!=last)
    { run=run->next;
      if (ISMARKED_V(run->end)) return 1;
    }
 ...
 
{
  ...
  RESETMARKS;
  while (--numnf3cycleedges >=0) MARKLO(nf3cycleedges[numnf3cycleedges]);
  ...
  for (i=0; i<nv; i++)
    if (degree[i]>=6)
      { run=end=firstedge[i];
        do
          { if (run==run->min && degree[run->end]>=6
                  && !ISMARKEDLO(run) && is_3banglecenter_notdouble(run))
              { list[counter]=run; counter++; }
            run=run->next;
          }
        while (run != end);
      }
 ...
 
{
 /* Currently on4[] is not used, but we could use it if we wanted. */
    ...
    if (nbtot > 1 && *next5_sw > 1) prune_edgelist(ext5_sw,next5_sw,nbtot);
}
...
#define SW53COL(e) (((long)degree[(e)->start] << 10) | (long)degree[(e)->end])
{
    ...
    for (i = 0; i < non3; ++i)
    {
        e1 = on3[i];
        e2 = e1->invers;
        if (degree[e1->start] >= 6 && degree[e1->end] >= 6 && e1 != e
                && wont_be_on_nf_3_cycle(e1))
        {
            col = SW53COL(e1);
            if (col > bestcol)
            {
...

{
    ...
    for (e1 = e->next->invers->prev->prev, k = degree[e->next->end]-4;
        ...
        for (e2 = e1->invers->next->next, l = degree[e1->end]-3;
                                                 --l >= 0; e2 = e2->next)
            if (ISMARKED_V(e2->end)) return TRUE;
...

{
    ...
    for (i = 0; i < nv; ++i)
    {
        ...
        do
        {
            if (e == e->min && is_in_5_bangle(e)) bang[nb++] = e;
            e = e->next;
        } while (e != elast);
    }
...

{
  ...
  last=e->prev->prev;
    ...
    last1=dummy->prev->prev;
    run1=dummy->next->next;
    if (ISMARKED_V(run1->end)) return 1;
    run1=run1->next;
    if (ISMARKED_V(run1->end)) return 1;
    while (run1!=last1)
      { run1=run1->next; if (ISMARKED_V(run1->end)) return 1; }
  while (run!=last)
    { run=run->next;
      dummy=run->invers;
      last1=dummy->prev->prev;
...

{
  ...
  if ((!ISMARKEDLO(e->next->min)) || (!ISMARKEDLO(e->prev->min))) return 0;
...

{
  ...
  RESETMARKS;
  while (--numnf4cycleedges >= 0) MARKLO(nf4cycleedges[numnf4cycleedges]);
  ...
  for (i=0; i<nv; i++)
    if (degree[i]>=6)
      { run=end=firstedge[i];
        do
          { if ((run==run->min) && (degree[run->end]>=6) && (!ISMARKEDLO(run))
                && is_4bangle_centre(run) && (!will_be_on_nf_3_cycle(run)))
            { list[counter]=run; counter++; }
            run=run->next;
          }
        while (run != end);
...

{
  ...
  last=e->prev->prev;
    ...
    last1=dummy->prev->prev;
    run1=dummy->next->next;   
    if (ISMARKED_V(run1->end)) return 0;
    run1=run1->next;
    if (ISMARKED_V(run1->end)) return 0;
    while (run1!=last1)
      { run1=run1->next; if (ISMARKED_V(run1->end)) return 0; }
  run=run->next;
    dummy=run->invers;
    last1=dummy->prev->prev;
    run1=dummy->next->next;
  ...
  
{
  ...
  e=e->invers; e1=e1->invers;

  run=e->next->next;
  if (degree[run->end]==5) MARK_V(run->end);
  run=run->next;
  if (degree[run->end]==5) MARK_V(run->end);
  last=e->prev->prev;
  while (run!=last)
...
{ 
  ...
  for (i = numnf4cycleedges; --i >= 0; )
    { MARKLO(nf4cycleedges[i]); MARKLO(nf4cycleedges[i]->invers); }
...

{
    ...
    if (nbtot > 1 && k > 1) prune_edgelist(ext5_sw,&k,nbtot);
    ...
    *next5_sw = k;
    ...
    if (nv < maxnv && on4 && non4 >= 6)
    {
        ...
        for (i = 0; i < *next5_5; ++i)
        {
            if (ext5_51[i] < ext5_52[i]) { e1 = ext5_51[i]; e2 = ext5_52[i]; }
            else                         { e1 = ext5_52[i]; e2 = ext5_51[i]; }
            ...
            for ( ; nb1 < nblim; nb1 += MAXE, nb2 += MAXE)
                if (*nb1 < *nb2)
                {
                    if (*nb1 < e1 || (*nb1 == e1 && *nb2 < e2)) break;      
                }
                else
                {
                    if (*nb2 < e1 || (*nb2 == e1 && *nb1 < e2)) break;      
                }
...

{
  ...
  e1=e->prev->prev->invers; e2=e->next->next->invers;
  e=e->invers;
  ...
  run1=run1->next;
  if (ISMARKED_V(run1->end)) return 1;
  while (run1!=last1) { run1=run1->next; if (ISMARKED_V(run1->end)) return 1; }
  run=run->next;
  dummy=run->invers;
  last1=dummy->prev->prev;
  run1=dummy->next->next;
...

{
  ...
  if (!ISMARKEDLO(e->next->min) || !ISMARKEDLO(e->prev->min)) return 0;
  dummy=e->invers;
  if (!ISMARKEDLO(dummy->next->min) || !ISMARKEDLO(dummy->prev->min)) return 0;
  if (!common_5_endpoint(e->next,e->prev)) return 0;
  if (will_be_on_nf4_5red(e)) return 0;
...

{
    ...
    ng = 0;
    if (legal_5_min5_reduction(ref,on4,non4)) good[ng++] = ref;
    if (legal_5_min5_reduction(ref->invers,on4,non4)) good[ng++] = ref->invers;
    ...
    if (degree[i] == 5)
    {
        ...
        do
        {
            if (degree[e->end] >= bestdeg && e != ref && e != ref->invers
                                         && legal_5_min5_reduction(e,on4,non4))
            {
...
#define FIND_EXTENSIONS_MIN5c4 find_extensions_min5c4
    ...
    if (nv == maxnv)
    {
        if (pswitch) startpolyscan(nbtot,nbop);   /* Saves the group! */
        else         got_one(nbtot,nbop,4);
 
        if (minconnec < 4 && non4 >= 6) scanmin5c3_0(nbtot,nbop,on4,non4);
    }
...

{
    ...
    {
        if (pswitch) startpolyscan(nbtot,nbop);   /* Saves the group! */
        else         got_one(nbtot,nbop,5);
        if (nbangles > 0) scanmin5c4_0(nbtot,nbop,bangle,nbangles);
        return;
    }
    ...
    for (i = 0; i < nextA; ++i)
    {
...
#endif  
        {
            ...
            if (ngood_ref+ngood_mir_ref > 0)
            {
                ...
                if (is_in_5_bangle(extAred)) newbang[newnbang++] = extAred;
    
                if (nv == maxnv)
                {
                    k = 0;
                    for (j = 0; j < newnbang; ++j)
                        if (degree[newbang[j]->start] >= 6
                                              && degree[newbang[j]->end] >= 6)
                            newbang[k++] = newbang[j];
                    newnbang = k;
                }
...
/* Make a cube using the first 24 edges */
{
   ...
   EDGE *buffer;
    ...
    for (i=0; i<8; i++)
      { buffer=edges+3*i;
        firstedge[i]=buffer; degree[i]=3;
        buffer->next=buffer+1; buffer->prev=buffer+2; buffer->start=i;
        buffer++; buffer->next=buffer+1; buffer->prev=buffer-1; buffer->start=i;
        buffer++; buffer->next=buffer-2; buffer->prev=buffer-1; buffer->start=i;
      }
...

{
    ...
    for (i=0; i<numold; i++, old++)
      { e = *old; 
        if (degree[e->start] > minpolydeg && degree[e->end] > minpolydeg
             && (e->left_facesize == 4 || e->invers->left_facesize == 4))
            newe[counter++]=e;
      }
...
#define REJECTB(x) {++degree[v1]; ++degree[v2]; return x;}
#define ORTESTB(e) {col = DEGENDB(e); if (col > maxcol) \
  {if (OROKB(e)) REJECTB(2)} \
   else if (col==maxcol&&OROKB(e)) good_or[ng_or++]=e;}
#define INVTESTB(e) {col = DEGENDB(e); if (col > maxcol) \
  {if (INVOKB(e)) REJECTB(2)} \
   else if (col==maxcol&&INVOKB(e)) good_inv[ng_inv++]=e;}
#define ORCOLB(e) (((long)degree[e->invers->prev->end] << 10)  \
                      + e->left_facesize)
#define INVCOLB(e) \
 (((long)degree[e->invers->next->end] << 10) + e->invers->left_facesize)
 ...
 /* Now old edges that will be on a largest face (if *edel is deleted)
    ...
    e1 = edel->prev; v3 = e1->end;
    e4 = edel->next; v4 = e4->end;
    e3 = edel->invers->prev; v6 = e3->end;
    e2 = edel->invers->next; v5 = e2->end;
    ...
    if (maxface == 6)
    {
     /* Recall from above that all degrees are <= maxdeg whenever
        ...
        if (ISNEQADJ(v3,v6))
        {
            if (degree[v3] == maxdeg)
            {
                if (degree[v1] > maxcol) REJECTB(2)
                else if (degree[v1] == maxcol) good_inv[ng_inv++] = e1->invers;
                if (degree[v5] > maxcol) REJECTB(2)
                else if (degree[v5] == maxcol) good_or[ng_or++] = e5;
            }

            if (degree[v6] == maxdeg)
            {
                if (degree[v4] > maxcol) REJECTB(2)
                else if (degree[v4] == maxcol) good_or[ng_or++] = e6;
                if (degree[v2] > maxcol) REJECTB(2)
                else if (degree[v2] == maxcol) good_inv[ng_inv++] = e3->invers;
            }
        }
        ...
        if (ISNEQADJ(v4,v5))
        {
            if (degree[v4] == maxdeg)
            {
                if (degree[v6] > maxcol) REJECTB(2)
                else if (degree[v6] == maxcol) good_inv[ng_inv++] = e6->invers;
                if (degree[v1] > maxcol) REJECTB(2)
                else if (degree[v1] == maxcol) good_or[ng_or++] = e4->invers;
            }

            if (degree[v5] == maxdeg)
            {
                if (degree[v2] > maxcol) REJECTB(2)
                else if (degree[v2] == maxcol) good_or[ng_or++] = e2->invers;
                if (degree[v3] > maxcol) REJECTB(2)
                else if (degree[v3] == maxcol) good_inv[ng_inv++] = e5->invers;
            }
        }
    ...
    else
    {
        ...
        do
        {
            if (ISNEQADJ(ea->end,eb->end))
            {
                da = DEGENDB(ea);
                db = DEGENDB(eb);
                if (da > maxdeg || db > maxdeg) REJECTB(2);

                if (da == maxdeg)
                {
                    if (degree[ea->start] > maxcol) REJECTB(2)
                    else if (degree[ea->start] == maxcol)
                        good_or[ng_or++] = ea->invers;
                }
                if (db == maxdeg)
                {
                    if (degree[eb->start] > maxcol) REJECTB(2)
 ...
 /* Now test old edges still on max faces */
    ...
    for (i = oldmaxlist0; i < oldmaxlist1; ++i)
    {
        ...
        if (degree[ee->start] > maxdeg)
        {
            if (INVOKB(ee)) REJECTB(2);
            ee = ee->prev;
            if (OROKB(ee)) REJECTB(2);
        }
        else if (degree[ee->start] == maxdeg)
        {
            INVTESTB(ee);
    ...
    if (newconnec < minpolyconnec) REJECTB(1);
 ...
 /* Now we have complete success!  Just prune the lists a bit and
    ...
    *ncan_inv = nc_inv;
    ...
    maxcol = 0;
    for (i = 0; i < nc_or; ++i)
        if (ORCOLB(good_or[i]) > maxcol) maxcol = ORCOLB(good_or[i]);
    for (i = 0; i < nc_inv; ++i)
        if (INVCOLB(good_inv[i]) > maxcol) maxcol = INVCOLB(good_inv[i]);

    for (i = 0, k = 0; i < nc_or; ++i)
        if (ORCOLB(good_or[i]) == maxcol) good_or[k++] = good_or[i];
    *ncan_or = k;
    for (; i < ng_or; ++i)
    {
    ...
    *ngood_or = ng_or = k;
    ...
    for (i = 0, k = 0; i < nc_inv; ++i)
        if (INVCOLB(good_inv[i]) == maxcol) good_inv[k++] = good_inv[i];
    *ncan_inv = k;
    for (; i < ng_inv; ++i)
    {
 ...
 * the actual connectivity. */
{
    ...
    for (i = 0; i < noldfeas; ++i)
    {
        ...
        e = oldfeas[i];
        if (e->left_facesize != 4) e = e->invers;
        if (e->invers->left_facesize < oldmaxface-2
                      || e->invers->left_facesize == maxfacesize)
            continue;
   ...
   Possible values are 2 and 3 */
{
  ...
  if (lastdeg>=0) { if (degree[lastdeg]==2)  return 2;}
      ...
      if (degree[othervertex]>3)
        {
          ...
          while (run!=runend)
            { if (run->invers->prev->end==othervertex) return 2;
            run=run->next; }
        }
    }
  ...
  for (i=0;i<nv;i++) if (degree[i]==2) { lastdeg=i; return 2;}
   ...
   and n-1 vertices of degree 1 */
{
   ...
   if (n==1) { nv=1; ne=0; firstedge[0]=NULL; return; }
   ...
   if (n>2) buffer->next=buffer+1; else buffer->next=buffer;
...

{
   ...
   for (i=0;i<=n-1;i++)
     ...
     if (i>0) buffer->prev=buffer->next=buffer-1;
     else buffer->prev=buffer->next=edges+2*n-1;
     if (i<n-1) bufferb->prev=bufferb->next=bufferb+1;
     else bufferb->prev=bufferb->next=edges;
     }
...
*/
    ...
    */
    ...
    for (n=8; n<MAXN-2; n++)
      { start=quadr_P2(n);
        ...
        run=start;
        run->start=n; run->prev=start+7;

        run=start+1; run->end=n;

        run=start+2; 
        run->start=n+1; run->prev=start+4; run->next=start+6;
        
        run=start+3; run->end=n+1;

        run=start+4;
        run->start=n+1; run->prev=start+6; run->next=start+2;

        run=start+5; run->end=n+1;

        run=start+6;
        run->start=n+1; run->end=n; run->prev=start+2; run->next=start+4;

        run=start+7;
        run->start=n;  run->end=n+1;  run->next=start;
    ...
    */
    ...
    for (n=8; n<MAXN-4; n++)
      ...
      for (i=0; i<4; i++)
        { run=start+3*i;
          run->next=run+1; run->prev=run+2; run->start=n+i;
          run++; run->next=run+1; run->prev=run-1; run->start=n+i;
          run++; run->next=run-2; run->prev=run-1; run->start=n+i;
      }

      run=start; run->invers=start+12; run->min=run;
      run=start+1; run->invers=start+3; run->min=run; run->end=n+1;
      run=start+2; run->invers=start+11; run->min=run; run->end=n+3;
      run=start+3; run->invers=start+1; run->min=run->invers; run->end=n;
      run=start+4; run->invers=start+13; run->min=run;
      run=start+5; run->invers=start+6; run->min=run; run->end=n+2;
      run=start+6; run->invers=start+5; run->min=run->invers; run->end=n+1;
      run=start+7; run->invers=start+14; run->min=run;
      run=start+8; run->invers=start+9; run->min=run; run->end=n+3;
      run=start+9; run->invers=start+8; run->min=run->invers; run->end=n+2;
      run=start+10; run->invers=start+15; run->min=run;
      run=start+11; run->invers=start+2; run->min=run->invers; run->end=n;
      run=start+12; run->invers=start; run->min=run->invers; run->end=n;
      run=start+13; run->invers=start+4; run->min=run->invers; run->end=n+1;
      run=start+14; run->invers=start+7; run->min=run->invers; run->end=n+2;
      run=start+15; run->invers=start+10; run->min=run->invers; run->end=n+3;
...

{
  ...
  start=quadr_P2(nv);
  dummy=start+1;
  start->end=dummy->start=end3; degree[end3]++;
  e6i->next=e5i->prev=dummy; dummy->prev=e6i; dummy->next=e5i;

  dummy=start+3;
  dummy->start=(dummy-1)->end=end2; firstedge[end2]=dummy;
  e5->next=e4->prev=dummy; dummy->next=e4; dummy->prev=e5;

  dummy=start+5;
  dummy->start=(dummy-1)->end=end1; degree[end1]++;
  e3->next=e2->prev=dummy; dummy->next=e2; dummy->prev=e3;

  dummy=start+7;
  e->end=e1->start=nv;
  start->next=dummy->prev=e1; e1->next=dummy; e1->prev=start;

  degree[nv]=degree[nv+1]=3;
  firstedge[nv]=start; firstedge[nv+1]=start+2;
...
{  register EDGE *e1, *e2, *e3, *e4, *e5, *e5i, *e6i;
   ...
   e6i->next=e5i; e5i->prev=e6i; degree[end3]--; firstedge[end3]=e5i;

   e5->next=e4->prev=e1; e1->next=e4; e1->prev=e5; 
   e1->start=e->end=end2; firstedge[end2]=e1;

   e3->next=e2; e2->prev=e3; degree[end1]--; firstedge[end1]=e2;
}
...

{
  ...
  vertex=e->start;
  dummy=start+12;
  start->end=dummy->start=vertex; degree[vertex]++;
  e->next=run->prev=dummy; dummy->next=run; dummy->prev=e;

  e=run->invers;
  run=e->next;
  vertex=e->start;
  dummy=start+15;
  (start+10)->end=dummy->start=vertex; degree[vertex]++;
  e->next=run->prev=dummy; dummy->next=run; dummy->prev=e;
  ...
  vertex=e->start;
  dummy--;
  (start+7)->end=dummy->start=vertex; degree[vertex]++;
  e->next=run->prev=dummy; dummy->next=run; dummy->prev=e;
  ...
  vertex=e->start;
  dummy--;
  (start+4)->end=dummy->start=vertex; degree[vertex]++;
  e->next=run->prev=dummy; dummy->next=run; dummy->prev=e;

  firstedge[nv]=start;     degree[nv]=3;
  firstedge[nv+1]=start+3; degree[nv+1]=3;
  firstedge[nv+2]=start+6; degree[nv+2]=3;
  firstedge[nv+3]=start+9; degree[nv+3]=3;
...

{
  ...
  dummy=e->next->next;
  start=e->start;
  degree[start]--; firstedge[start]=e;
  e->next=dummy; dummy->prev=e;
...

{
  ...
  if (degree[end1]<degree[end2])
    ...
    run=e->next; last=e->prev;
    while (run!=last)
      { if (run->invers->prev->end==end1) return 0;
        run=run->next; }
    }
...
{ EDGE *run, *last;
 ...
 do { if (run->end==w) return 0; run=run->next; } while (run!=last);
 ...
 if (run->invers->prev->end==w1) return 0;
 /* cannot be w2 */
 for (run=run->next; run!=last; run=run->next)
   { buffer=run->invers->prev->end;
   if (buffer==w1) return 0;
   if (buffer==w2) return 0;
   }

 /* the last one cannot be w1 */
 if (run->invers->prev->end==w2) return 0;
...
{ EDGE *run, *last;
 ...
 do { if (run->end==w) return FALSE; run=run->next; } while (run!=last);
...

{
    ...
    do
    {
        ...
        do
        {
            if (ISMARKED_V(e2->end)) return FALSE;
            e2 = e2->next;
        } while (e2 != e2last);
...

{
  ...
  if (degree[e->next->end]!=3) return 0;
  run=e->invers->prev;
  if (degree[run->end]!=3) return 0;
  ...
  while (run!=last)
    { if (run->invers->prev->end==v1) return 0;
    run=run->next; }
  ...
  run=e->next;
  while (run!=last)
    { if (run->invers->prev->end==v2) return 0;
    run=run->next; }
     ...
     /* P1 extension for trivial group */
        ...
        if (degree[i] >= 4)
        {
            ...
            if (i == vx)
                do
                {
                    if (will_be_3_connected(e)) extP1[k++] = e;
                    e = e->next;
                } while (e != elast);
            else
                do
                {
                    if (ISMARKED_V(e->prev->end) || ISMARKED_V(e->next->end)
                                                  || degree[e->end] >= maxdeg)
                    {
                        if (will_be_3_connected(e)) extP1[k++] = e;
                    }
...
/****************************************************************************/
static void
find_extensions_multiquad(int nbtot, int nbop, EDGE *extD1[], int *nextD1,
                       EDGE *extP0[], int *nextP0, EDGE *extP1[], int *nextP1)
...
*/
{
    ...
    int deg1, deg2;
    int deg1_list[2];
    int max_neighbourdeg;
    int second_neighbourdeg;
...
#define VCOLP0(i, j) (degree[i]<degree[j] ? \
 (degree[j]<<10)+degree[i] : (degree[i]<<10)+degree[j])
    ...
    deg1 = 0;
    deg2 = 0;
    max_neighbourdeg = 0; // The highest degree that a neighbour
                          // of a degree 1 vertex can have
    second_neighbourdeg = 0; // The second highest degree that a neighbour
                             // of a degree 1 vertex can have
    for (i = nv - 1; i >= 0 && deg1 < 3; i--)
    /* Once three vertices of degree 1 have been found we have enough information.
     As the canonical D1 always has the neighbour with the largest degree
     and we traverse the vertices from last added to first added,
     max_neighbourdegree and second_neighbourdeg are correct */
    {
    ...
    if (nbtot == 1)
    {
        ...
        for (l = 0; l < nv; ++l)
        {
            ...
            do
            {
                if (degree[l] == 1)
                {
                    ...
                    {
                        // The degree of the new neighbour will be 3.s
                        // If there is a larger one e is not canonical
                        extD1[k++] = e;
                    }
                }
                else if (degree[l] + 2 >= max_neighbourdeg)
                    extD1[k++] = e;
                    // if degree[l] != 1 then maxdeg will remain the same or larger
                    // after extension
        ...
        *nextD1 = k;
        ...
        else
        {
            ...
            if (deg1 == 1)
            {
                /* there are two possible edges that can be chosen. However,
                  they extend to the same graph, with a different marked edge.
                  One of them always has a lower degree of ->end
        ...
        /* P1 extension for trivial group */
        //P1 can remove at most two vertices of degree 1 and of degree 2
        // with different neighbours so if there are three the resulting
        // graph can be constructed with D1 or P0
        if (deg1 + deg2 > 2)
            *nextP1 = 0;
        else
        {
            k = 0;
            if (deg1 == 2)
            {//two vertices of degree 1 must be removed
            ...
            {//one vertex of degree 1 must be removed
                e = firstedge[deg1_list[0]]->invers;
                if(degree[e->start] >= 4)
                {
                    extP1[k++] = e->next;
                ...
                for (l = 0; l < nv; ++l)
                    if (degree[l] >= 4)
                    {
                        ...
                        do
                        {
                            i = e->next->end;
                            j = e->prev->end;
                            if (i != j) //no parallel edges in face
                                extP1[k++] = e;
                            e = e->next;
                        } while (e != elast);
                    }
    ...
    else
    {
        ...
        for (l = 0; l < ne; ++l)
            if (!ISMARKED(nb0[l]))
            {
                ...
                else if (degree[e->start] + 2 >= max_neighbourdeg)
                    extD1[k++] = e;
        ...
        *nextD1 = k;
        ...
        if (deg1 > 1 && nv != 3)
            *nextP0 = 0;
                // Testing for deg1 == 1 is almost useless here.
                // In that case there are almost certainly no automorphisms
        else
        {
            k = 0;
            RESETMARKS;
    ...
    if (degree[i] == 3)
    {
        ...
        do
        {
            if (degree[e->next->end] >= 4 && degree[e->prev->end] >= 4
                                      && legal_P1_reduction(e)) return TRUE;
            e = e->next;
        } while (e != elast);
    }
...
#define QMIRCOL(e) (((long)degree[e->next->end]<<10)+(long)degree[e->prev->end])
    ...
    *ngood_mir_ref = nmir;
    ...
    e = *hint;
    if (e && !ISMARKEDLO(e->invers)
             && degree[e->start] == 3 && degree[e->end] >= maxdeg
             && degree[e->next->end] >= 4 && degree[e->prev->end] >= 4
             && legal_P1_reduction(e))
    {
    ...
    {
        e = prevP1[i];
        if (!ISMARKEDLO(e->invers) && degree[e->end] >= maxdeg
                 && degree[e->start] == 3
                 && degree[e->next->end] >= 4 && degree[e->prev->end] >= 4
                 && legal_P1_reduction(e))
        {
    ...
    for (i = 0; i < nv; ++i)
        if (degree[i] > maxdeg)
        {
            ...
            do
            {
                ...
                {
                    ee = e->invers;
                    if (degree[ee->next->end] >= 4
                                && degree[ee->prev->end] >= 4
                                && legal_P1_reduction(ee))
                    {
                        *ngood_ref = *ngood_mir_ref = 0;
                        *hint = e;
                        return;
...
/* The P0-operation with reference edge *ref has just been performed.
   ...
   *ngood_ref and *ngood_mir_ref might each be 0-1.  If they are
   ...
   If there is a vertex of degree 2 its neighbours must be different
   for it to be a candidate.
*/
...
#define VCOLPD(di,dj) (di<dj ? (dj<<10)+di : (di<<10)+dj)
    ...
    if (d1 >= d2) //this edge is a candidate
    {
...
/* The P1-operation with reference edge *ref has just been performed.
   ...
   *ngood_ref and *ngood_mir_ref might each be 0-1.  If they are
   ...
   If there is a vertex of degree 3 two of its neighbours must be different
   for the other edge to be a candidate.
*/
                        ...
                        if (legal_P3_reduction(e->prev))
                        {
                            *ngood_ref = *ngood_mir_ref = 0;
                            return;
                                           ...
                                           && legal_P3_reduction(e->prev))
                    {
                        if (col1 == maxcol) good_or[nor++] = e;
                        if (col2 == maxcol) good_mir[nmir++] = e;
                    }
                }
                e = e->next;
            } while (e != elast);
                        ...
                        if (legal_P3_reduction_min3(e->prev))
                        {
                            *ngood_ref = *ngood_mir_ref = 0;
                            return;
                                        ...
                                        && legal_P3_reduction_min3(e->prev))
                    {
                        if (col1 == maxcol) good_or[nor++] = e;
                        if (col2 == maxcol) good_mir[nmir++] = e;
                    }
...

{
    ...
    if (nv == maxnv)
    {
        if (pswitch) startbipscan(nbtot,nbop,3);
        else         got_one(nbtot,nbop,3);
        return;
    }
...

{
    ...
    if (!(PRE_FILTER_QUAD_MIN3)) return;
#endif
...

{
    ...
    if (nv == maxnv)
    {
        got_one(nbtot,nbop,1);  /* Third arg is connectivity but
                                       we don't know it. */
        return;
    }
...
#ifdef PRE_FILTER_MULTIQUAD
    if (!(PRE_FILTER_MULTIQUAD)) return;
#endif

#ifndef FIND_EXTENSIONS_MULTIQUAD
#define FIND_EXTENSIONS_MULTIQUAD find_extensions_multiquad
#endif
...
#ifdef FAST_FILTER_MULTIQUAD
        if (FAST_FILTER_MULTIQUAD)
#endif
        {
...

{
    ...
    for (i = 1; !badargs && i < argc; ++i)
    {
        ...
        for (j = 1; arg[j] != '\0'; ++j)
            ...
            else if (arg[j] == 'e')
            {
                ...
                if (numbounds == 1) edgebound[1] = edgebound[0];
                if (edgebound[1] == 0) edgebound[1] = MAXE/2;
            }
    ...
    if (oswitch || Vswitch || oneswitch) Gswitch = TRUE;
...
*/
{
    ...
    if (aswitch + gswitch + sswitch + uswitch + Eswitch + Tswitch >= 2)
    {
        fprintf(stderr,">E %s: -a, -g, -s, -u, -E, -T are incompatible\n",cmdname);
        exit(1);
    }

    if (!uswitch)   /*UPDATE for -E/-T and reconconsider */
    {
    ...
    if (outfilename == NULL)
    {
        ...
        zeroswitch || oneswitch ||
        aswitch || Tswitch || gswitch || sswitch ? "w" : "wb")) == NULL)
    {
        fprintf(stderr,
    ...
    if (zeroswitch)  /* implied by oneswitch */
        ...
        write_graph = write_dual_graph = write_double_code_int;
    else if (aswitch)
    {
    ...
    if (!uswitch && !aswitch && !Tswitch)
    {
        if ((!zeroswitch && !Tswitch && !hswitch && !gswitch && !sswitch && !Eswitch &&
...
/* All the cases not handled in the other dispatch routines. */
{
    ...
    if (minimumdeg <= 0) minimumdeg = 3;
    if (minconnec <= 0)  minconnec = minimumdeg;
    startingsize = 4;
    ...
    else if (maxnv == 3 && !Aswitch)
    {
        ...
        {   
            ...
            def[1] = nv - degree[1];
            def[2] = nv - degree[2];
            canon(def,numbering,&nbtot,&nbop);
#ifdef FAST_FILTER_SIMPLE
        if (FAST_FILTER_SIMPLE)
#endif 
            got_one(nbtot,nbop,1);
    ...
    if (vswitch && minconnec < 3 && !xswitch)
    {
        ...
        {
            if (i > minconnec) fprintf(msgfile," ");
            PRINTBIG(msgfile,nout[i]);
            if (i == 3) fprintf(msgfile," (3-5)-conn");
            else        fprintf(msgfile," %d-conn",i);
            if (i < 3) fprintf(msgfile,";");
        }
        fprintf(msgfile,"\n");
   ...
   larger to achieve this, but put back before this procedure returns. */ 
{
    ...
    if (minimumdeg < minconnec) minimumdeg = minconnec;

    if (dswitch) strcpy(outtypename,"duals of disk triangulations");
    else         strcpy(outtypename,"disk triangulations");

    open_output_file();

    for (i = 0; i <= MAXN; ++i) nout_p[i] = nout_p_op[i] = 0;
...
/* Case of minimum degree >= 4, except for polygons and polytopes. */
{
    ...
    INCOMPAT(minconnec==4&&xswitch,"-c4","-x");
...
/* Case of minimum degree == 5, except for polygons and polytopes. */
{
    ...
    INCOMPAT(minconnec==5&&xswitch,"-c5","-x");
    ...
    hint = (maxnv < 35 ? 28 : maxnv < 38 ? 29 : 30);
...
/* Polytopes.  Main options are -m4, -m5 and neither. */
{
  ...
  /* Polytopes are made by first making 3-connected triangulations,
    ...
    if (minconnec > minimumdeg) minimumdeg = minconnec;
    if (minpolyconnec > minpolydeg) minpolydeg = minpolyconnec;
...
/* INCOMPAT(minconnec == 3 && xswitch,"-c3","-x");  changed 19/2/08 */
    ...
    if (edgebound[0] < 0) edgebound[0] = 0;
    if (edgebound[1] < 0) edgebound[1] = maxundir;
    if (edgebound[1] > maxundir) edgebound[1] = maxundir;
    if (maxfacesize <= 0) maxfacesize = MAXE;
    ...
    if (maxnv >= 4 && (edgebound[0] == maxundir || maxfacesize == 3))
    {
        ...
        if      (minimumdeg == 4) min4_dispatch();
        else if (minimumdeg == 5) min5_dispatch();
    ...
    {
        if (res > 0) return;  /* new at version 5.5 */
        if (maxnv <= 3 && minpolydeg == 1  && minpolyconnec == 1
                            && edgebound[0] <= 2*maxnv-2
        ...
        if (maxnv == 3 && minpolydeg <= 2 && minpolyconnec <= 2
                            && edgebound[0] <= 6
                            && edgebound[1] >= 6)
        {   
            make_cycle(maxnv);
    ...
    if (vswitch)
    {
        ...
        if (!xswitch && minpolyconnec < 3)
        {
            ...
            {
                if (i > minpolyconnec) fprintf(msgfile," ");
                PRINTBIG(msgfile,nout[i]);
                if (i == 3) fprintf(msgfile," (3-5)-conn");
                else        fprintf(msgfile," %d-conn",i);
                if (i < 3) fprintf(msgfile,";");
...
/* Eulerian triangulations. */
{
    ...
    if (dswitch) strcpy(outtypename,"bipartite cubic graphs");
    else         strcpy(outtypename,"eulerian triangulations");
...
/* Simple quadrangulations. */
{
    ...
    if (minimumdeg <= 0) minimumdeg = 3;
    if (minconnec <= 0) minconnec = 3;
    if (minimumdeg==2 && minconnec>=3) minimumdeg = 3;
    ...
    if (dswitch) strcpy(outtypename,"quartic graphs");
    else         strcpy(outtypename,"quadrangulations");
...
/* General quadrangulations. */
{
    ...
    CHECKRANGE(maxnv,"n",startingsize,MAXN);
    CHECKRANGE(minconnec,"-c",1,1);
    CHECKRANGE(minimumdeg,"-m",1,1);

    INCOMPAT(Aswitch,"-Q","-A");
    INCOMPAT(xswitch,"-Q","-x");
    INCOMPAT(polygonsize>=0,"-Q","-P");
    INCOMPAT(tswitch,"-Q","-t");
    INCOMPAT(edgebound[0]>=0,"-Q","-e");
    INCOMPAT(maxfacesize>=0,"-Q","-f");
    ...
    nbtot = 2;                            // remove duplicates
    nbop = 1;
    scanmultiquadrangulations(nbtot,nbop);

#ifdef STATS
    fprintf(msgfile,"Counts by min degree: ");
    PRINTBIG(msgfile,nummindeg[1]);
    fprintf(msgfile," ");
    PRINTBIG(msgfile,nummindeg[2]);
    fprintf(msgfile," ");
    PRINTBIG(msgfile,nummindeg[3]);
...
/* General bipartite graphs.  Main options are -m# -c# for #=1,2,3.  */
{
  ...
  /* Bipartite graphs are made by first making quadrangulations,
    ...
    if (edgebound[0] < 0) edgebound[0] = 0;
    if (edgebound[1] < 0) edgebound[1] = maxundir;
    if (edgebound[1] > 2*maxnv-4) edgebound[1] = maxundir;
    if (maxfacesize <= 0) maxfacesize = MAXE;
    ...
    if (dswitch) strcpy(outtypename,"eulerian graphs");
    else         strcpy(outtypename,"bipartite graphs");

    open_output_file();

    for (i = 0; i <= maxundir; ++i) nout_e[i] = nout_e_op[i] = 0;
...
main(int argc, char *argv[])
{
    ...
    for (i = 0; i < 6; ++i) nout[i] = nout_op[i] = 0;
    nout_V = 0;
    ...
    else if (bswitch)                           eulerian_dispatch();
    else if (qswitch)                           quadrangulation_dispatch();
    else if (Qswitch)                           multiquadrangulation_dispatch();
