...
def faces_by_vertices(g):
    ...
    d = {}
    for key, val in g.get_embedding().items():
...
def is_well_colored(graph):
    ...
    while is_end_of_job is False and i_vertex < len(vertices):
        ...
        if len(edges) != 3:
            ...
            # The colors (keys)around this vertex are: [edges[0][2], edges[1][2], edges[2][2]]
            if logger.isEnabledFor(logging.DEBUG): logger.debug("vertex: %s, edges: %s, elements: %s", vertex, edges, [edges[0][2], edges[1][2], edges[2][2]])

            if (edges[0][2] == edges[1][2]) or (edges[0][2] == edges[2][2]) or (edges[1][2] == edges[2][2]) or (edges[1][2] == edges[0][2]) or (edges[2][2] == edges[0][2]) or (edges[2][2] == edges[1][2]):
                is_well_colored = False
                is_end_of_job = True
            else:
                i_vertex += 1

    if logger.isEnabledFor(logging.INFO): logger.info("END: is_well_colored (%s)", is_well_colored)
...
def export_output_graph(graph_to_export, name_of_file_without_extension):
    """
    Export graph
    ...
    graph_to_export.graphviz_to_file_named(name_of_file_without_extension + ".tmp.dot", edge_labels=True, vertex_labels=False)
    logger.info("File saved: %s", name_of_file_without_extension)

    # Replace "label" with "color". Necessary to be used from Gephy
    filedata = None
    with open(name_of_file_without_extension + ".tmp.dot", 'r') as file:
        filedata = file.read()
    ...
    os.remove(name_of_file_without_extension + ".tmp.dot")
